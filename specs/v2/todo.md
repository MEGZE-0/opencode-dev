...
The nexusflow server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "nexusflow" instance like in that post i showed
- nexusflow instance has stuff like `nexusflow.session.prompt()` or
  `nexusflow.tool.register({...})`
