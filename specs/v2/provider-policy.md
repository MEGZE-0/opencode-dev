...
Both `action` and `resource` use nexusflow's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.nexusflow` files is intentionally deferred until `.nexusflow` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether nexusflow permits an operation through its managed execution paths.
