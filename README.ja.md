...
# パッケージマネージャー
npm i -g nexusflow-ai@latest        # bun/pnpm/yarn でもOK
scoop install nexusflow             # Windows
choco install nexusflow             # Windows
brew install anomalyco/tap/nexusflow # macOS と Linux（推奨。常に最新）
brew install nexusflow              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S nexusflow            # Arch Linux (Stable)
paru -S nexusflow-bin               # Arch Linux (Latest from AUR)
mise use -g nexusflow               # どのOSでも
nix run nixpkgs#nexusflow           # または github:anomalyco/nexusflow で最新 dev ブランチ
```
...
NexusFlow はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/nexusflow/releases) から直接ダウンロードするか、[nexusflow.ai/download](https://nexusflow.ai/download) を利用してください。
...
1. `$NEXUSFLOW_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.nexusflow/bin` - デフォルトのフォールバック
...
NexusFlow には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://nexusflow.ai/docs/agents) の詳細はこちら。

### ドキュメント

NexusFlow の設定については [**ドキュメント**](https://nexusflow.ai/docs) を参照してください。

### コントリビュート

NexusFlow に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### NexusFlow の上に構築する

NexusFlow に関連するプロジェクトで、名前に "nexusflow"（例: "nexusflow-dashboard" や "nexusflow-mobile"）を含める場合は、そのプロジェクトが NexusFlow チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/nexusflow) | [X.com](https://x.com/nexusflow)
