...
description: Drizzle schema, relations, query builder, migrations, dialects, and performance.
applies-to: ["database-architect", "nodejs-specialist", "supabase-specialist"]
---
...
- drizzle work must follow the local repo style before generic framework preferences.
...
~~~ts
export function drizzleRecipe1(input: string) {
...
~~~ts
export function drizzleRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function drizzleRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function drizzleRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function drizzleRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
