...
description: Hono routing, middleware, validation, OpenAPI, auth, error handling, and tests.
applies-to: ["nodejs-specialist", "api-architect", "security-specialist"]
---
...
- hono work must follow the local repo style before generic framework preferences.
...
~~~ts
export function honoRecipe1(input: string) {
...
~~~ts
export function honoRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function honoRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function honoRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function honoRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
