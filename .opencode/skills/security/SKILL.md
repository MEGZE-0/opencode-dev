...
description: OWASP, secure headers, JWT, hashing, injection, XSS, CSRF, rate limiting, and secrets.
applies-to: ["security-specialist", "api-architect", "devops-specialist"]
---
...
- security work must follow the local repo style before generic framework preferences.
...
~~~ts
export function securityRecipe1(input: string) {
...
~~~ts
export function securityRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function securityRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function securityRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function securityRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
