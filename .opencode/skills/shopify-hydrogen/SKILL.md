---
name: shopify-hydrogen
title: Hydrogen and Storefront API tactical patterns
version: 1.0.0
last-updated: 2026-06-12
description: Hydrogen and Storefront API tactical patterns for NexusFlow specialist agents.
tags: [nexusflow, tactical, specialist]
applies-to: ["ecommerce-specialist", "shopify-storefront-specialist"]
---

# Hydrogen and Storefront API tactical patterns

## Core Concepts

- This skill is tactical and should be loaded only when the task directly matches its domain.
- The steps below are ordered; do not skip earlier decision steps.
- Prefer existing repo conventions over generic framework defaults.

## Steps

1. Define Storefront API fragments before loaders.
2. Keep cart mutations in Remix actions.
3. Use SEO component data from product and collection payloads.

## Output Requirements

- State which step drove the recommendation.
- Include exact files, tools, metrics, or schema affected.
- Hand off work outside this domain to the right specialist.
