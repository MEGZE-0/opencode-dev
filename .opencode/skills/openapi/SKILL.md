...
description: OpenAPI 3.1, refs, schemas, security schemes, examples, clients, and Hono integration.
applies-to: ["api-architect", "nodejs-specialist", "tester"]
---
...
- openapi work must follow the local repo style before generic framework preferences.
...
~~~ts
export function openapiRecipe1(input: string) {
...
~~~ts
export function openapiRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function openapiRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function openapiRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function openapiRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
