...
description: GitHub Actions workflows, triggers, matrices, caching, artifacts, OIDC, and Bun setup.
applies-to: ["github-specialist", "devops-specialist", "tester"]
---
...
- github-actions work must follow the local repo style before generic framework preferences.
...
~~~ts
export function githubactionsRecipe1(input: string) {
...
~~~ts
export function githubactionsRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function githubactionsRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function githubactionsRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function githubactionsRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
