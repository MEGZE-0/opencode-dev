...
description: Electron and desktop app packaging, auto-update, native bridges, signing, and app telemetry.
applies-to: ["devops-specialist", "ui-designer", "security-specialist"]
---
...
- desktop work must follow the local repo style before generic framework preferences.
...
~~~ts
export function desktopRecipe1(input: string) {
...
~~~ts
export function desktopRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function desktopRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function desktopRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function desktopRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
