...
description: GA4, GTM, pixels, Klaviyo, server-side tagging, consent, no-PII, and attribution.
applies-to: ["analytics-specialist", "shopify-analytics-specialist", "media-buyer"]
---
...
- analytics work must follow the local repo style before generic framework preferences.
...
~~~ts
export function analyticsRecipe1(input: string) {
...
~~~ts
export function analyticsRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function analyticsRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function analyticsRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function analyticsRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
