...
description: Bun runtime, workspace, test, bundler, and API patterns.
applies-to: ["nodejs-specialist", "vite-specialist", "tester", "coder"]
---
...
- bun work must follow the local repo style before generic framework preferences.
...
~~~ts
export function bunRecipe1(input: string) {
...
~~~ts
export function bunRecipe2(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function bunRecipe3(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function bunRecipe4(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
...
~~~ts
export function bunRecipe5(input: string) {
  const value = input.trim()
  if (!value) throw new Error("input required")
  return value
}
