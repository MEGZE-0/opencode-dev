...
import { Config } from "@nexusflow/core/config"
import { ConfigProvider } from "@nexusflow/core/config/provider"
...
import { Project } from "@nexusflow/core/project"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "nexusflow.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "nexusflow.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "nexusflow.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "nexusflow.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "nexusflow.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "nexusflow.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .nexusflow configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".nexusflow"), { recursive: true })
            await fs.mkdir(path.join(directory, ".nexusflow"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "nexusflow.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "nexusflow.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "nexusflow.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "nexusflow.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".nexusflow", "nexusflow.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".nexusflow", "nexusflow.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(directories).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".nexusflow")),
              AbsolutePath.make(path.join(directory, ".nexusflow")),
            ])
