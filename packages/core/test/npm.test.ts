...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@nexusflow/acme")).toBe("@nexusflow/acme")
    expect(Npm.sanitize("@nexusflow/acme@1.0.0")).toBe("@nexusflow/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/nexusflow/acme.git"
    const expected = win ? "acme@git+https_//github.com/nexusflow/acme.git" : spec
