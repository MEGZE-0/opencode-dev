...
import { AISDK } from "@nexusflow/core/aisdk"
import { ModelV2 } from "@nexusflow/core/model"
import { PluginV2 } from "@nexusflow/core/plugin"
import { DynamicProviderPlugin } from "@nexusflow/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "nexusflow-provider-dynamic-"))
