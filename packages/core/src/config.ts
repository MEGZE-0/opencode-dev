...
export class Service extends Context.Service<Service, Interface>()("@nexusflow/v2/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["config.json", "nexusflow.json", "nexusflow.jsonc", "nexusflow.json", "nexusflow.jsonc"]
    ...
    const directories = locationIsGlobal
      ...
      : [
          globalDirectory,
          ...(yield* fs
            .up({ targets: [".nexusflow", ".nexusflow"], start: location.directory, stop: location.project.directory })
            .pipe(Effect.orDie))
            .toReversed()
    ...
    // global config, project files, then project config directory files.
