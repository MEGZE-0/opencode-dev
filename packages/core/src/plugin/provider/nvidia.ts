      ...
      "catalog.transform": Effect.fn(function* (evt) {
        for (const item of evt.provider.list()) {
          ...
          evt.provider.update(item.provider.id, (provider) => {
            provider.options.headers["HTTP-Referer"] = "https://nexusflow.ai/"
            provider.options.headers["X-Title"] = "nexusflow"
            provider.options.headers["X-BILLING-INVOKE-ORIGIN"] ??= "NexusFlow"
          })
        }
      }),
    }
