...
import { createOpencode, type ToolPart } from "@nexusflow/sdk"
...
console.log("🚀 Starting nexusflow server...")
const nexusflow = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await nexusflow.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new nexusflow session...")
    const { client, server } = nexusflow
    ...
    console.log("✅ Created nexusflow session:", createResult.data.id)
  ...
  console.log("📝 Sending to nexusflow:", message.text)
