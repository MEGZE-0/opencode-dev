...
import { type Session } from "@nexusflow/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("nexusflow://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("nexusflow://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("nexusflow://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("nexusflow://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("nexusflow://open-project")).toBeUndefined()
    expect(parseDeepLink("nexusflow://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "nexusflow://open-project?directory=/a",
      "nexusflow://other?directory=/b",
      "nexusflow://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("nexusflow://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("nexusflow://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("nexusflow://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("nexusflow://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "nexusflow://new-session?directory=/a",
      "nexusflow://open-project?directory=/b",
      "nexusflow://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NEXUSFLOW__: {
        deepLinks: ["nexusflow://open-project?directory=/a"],
      },
    } as unknown as Window & { __NEXUSFLOW__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["nexusflow://open-project?directory=/a"])
