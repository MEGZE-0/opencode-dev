...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.nexusflow.desktop.dev",
  beta: "ai.nexusflow.desktop.beta",
  prod: "ai.nexusflow.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.nexusflow.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `nexusflow.settings.dat` is special: it maps to the `nexusflow.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"nexusflow.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // nexusflow.settings.dat → the electron settings store ("nexusflow.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "nexusflow.global.dat").
  const storeName = filename === "nexusflow.settings.dat" ? "nexusflow.settings" : filename
