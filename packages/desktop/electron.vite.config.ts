...
const NEXUSFLOW_SERVER_DIST = "../nexusflow/dist/node"
const NEXUSFLOW_SERVER_MODULE = "\0nexusflow:server"
    ...
    plugins: [
      {
        ...
        enforce: "pre",
        resolveId(s) { if (s === "@lydell/node-pty") return nodePtyPkg }
      },
      {
      ...
      },
      {
        ...
        enforce: "pre",
        resolveId(id) { if (id === "virtual:nexusflow-server") return NEXUSFLOW_SERVER_MODULE },
        load(id) {
...
`
  ...
  renderer: {
    ...
    resolve: {
      alias: {
        "monaco-editor": fileURLToPath(new URL("./src/renderer/monaco-editor-stub.ts", import.meta.url)),
      },
    },
    build: {
      sourcemap: true,
