---
title: LSP Servers
description: NexusFlow integrates with your LSP servers.
---

NexusFlow can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

NexusFlow comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `NEXUSFLOW_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and nexusflow opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your nexusflow config.
...
```json title="nexusflow.json" {5-13}
{
  "$schema": "https://nexusflow.ai/config.json",
  "lsp": {
    "custom-lsp": {
