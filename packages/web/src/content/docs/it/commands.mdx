...
Crea `.nexusflow/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di NexusFlow o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di NexusFlow:
...
- Globale: `~/.config/nexusflow/commands/`
- Per progetto: `.nexusflow/commands/`
...
nexusflow include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
