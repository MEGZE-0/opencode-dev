...
Per diagnosticare problemi con NexusFlow, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `nexusflow --log-level DEBUG`.

---

## Archiviazione

nexusflow salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
NexusFlow Desktop esegue in background un server locale di NexusFlow (il sidecar `nexusflow-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `NexusFlow` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/nexusflow/nexusflow.jsonc`
...
NexusFlow puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che NexusFlow possa ricostruirla.

1. Chiudi completamente NexusFlow Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexusflow`
- **Linux**: elimina `~/.cache/nexusflow` (oppure esegui `rm -rf ~/.cache/nexusflow`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\nexusflow`

3. Riavvia NexusFlow Desktop.

---

### Risolvi problemi di connessione al server

NexusFlow Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `nexusflow.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `NEXUSFLOW_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `NEXUSFLOW_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, NexusFlow Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di NexusFlow.

---

### Notifiche non visualizzate

NexusFlow Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per NexusFlow nelle impostazioni del sistema operativo, e
...
1. Chiudi NexusFlow Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app NexusFlow Desktop):

- `nexusflow.settings.dat` (desktop default server URL)
- `nexusflow.global.dat` e `nexusflow.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con NexusFlow:
...
Per capire a quali modelli hai accesso, esegui `nexusflow models`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. nexusflow installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia nexusflow per reinstallare i pacchetti provider piu recenti

Questo forzera nexusflow a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
nexusflow rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
