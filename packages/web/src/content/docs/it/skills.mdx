...
Le skill degli agenti permettono a NexusFlow di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
NexusFlow cerca in queste posizioni:

- Config di progetto: `.nexusflow/skills/<name>/SKILL.md`
- Config globale: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, NexusFlow risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.nexusflow/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexusflow/skills/git-release/SKILL.md` cosi':
...
NexusFlow elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `nexusflow.json`:
...
**Per agenti integrati** (in `nexusflow.json`):
