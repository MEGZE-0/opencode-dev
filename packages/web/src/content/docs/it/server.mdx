---
title: Server
description: Interagisci con il server nexusflow via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `nexusflow serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client nexusflow.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `nexusflow.local` |
...
Imposta `NEXUSFLOW_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `nexusflow`, oppure imposta `NEXUSFLOW_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `nexusflow serve` sia per `nexusflow web`.
...
Quando esegui `nexusflow` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server nexusflow per interagire con nexusflow in modo programmatico.
:::

Questa architettura permette a nexusflow di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `nexusflow serve` per avviare un server standalone. Se la TUI di nexusflow e gia in esecuzione, `nexusflow serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di NexusFlow.
...
Il server nexusflow espone le seguenti API.
