---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di NexusFlow.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NexusFlow fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire NexusFlow avvia la TUI per la directory corrente.
...
Quando usi la TUI di NexusFlow, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a NexusFlow. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da NexusFlow. _Alias_: `/quit`, `/q`
...
Questo è separato da `nexusflow.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo NexusFlow attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `nexusflow.default`.
