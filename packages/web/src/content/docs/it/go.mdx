...
NexusFlow Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in NexusFlow. Ti abboni a NexusFlow Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare NexusFlow.
...
NexusFlow Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

NexusFlow Go funziona come qualsiasi altro provider in NexusFlow.

1. Accedi a **<a href={console}>NexusFlow Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `NexusFlow Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a NexusFlow Go.
:::
...
NexusFlow Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua NexusFlow config
utilizza il formato `nexusflow-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`nexusflow-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato NexusFlow Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con NexusFlow.
