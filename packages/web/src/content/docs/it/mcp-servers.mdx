...
Puoi aggiungere strumenti esterni a NexusFlow usando il _Model Context Protocol_, o MCP. NexusFlow supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [NexusFlow Config](https://nexusflow.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/nexusflow`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
NexusFlow gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, NexusFlow:
...
Se il server richiede autenticazione, NexusFlow ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `nexusflow mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, NexusFlow memorizzera' i token in modo sicuro in `~/.local/share/nexusflow/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in NexusFlow insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione NexusFlow come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare NexusFlow al tuo account Sentry.
