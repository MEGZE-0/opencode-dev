---
title: Plugin
description: Scrivi plugin per estendere NexusFlow.
---

I plugin ti permettono di estendere NexusFlow agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di NexusFlow.
...
- `.nexusflow/plugins/` - plugin a livello progetto
- `~/.config/nexusflow/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/nexusflow/node_modules/`.
...
1. Config globale (`~/.config/nexusflow/nexusflow.json`)
2. Config di progetto (`nexusflow.json`)
3. Directory plugin globale (`~/.config/nexusflow/plugins/`)
4. Directory plugin di progetto (`.nexusflow/plugins/`)
...
NexusFlow esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di nexusflow per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere nexusflow.
...
:::note
Se usi l'app desktop di NexusFlow, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a nexusflow di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a nexusflow:
...
L'helper `tool` crea uno strumento personalizzato che nexusflow può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in nexusflow insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://nexusflow.ai/docs/sdk) per i dettagli.
