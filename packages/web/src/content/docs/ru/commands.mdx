...
Создайте `.nexusflow/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию nexusflow или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем nexusflow [config](/docs/config):
...
- Глобальный: `~/.config/nexusflow/commands/`
- Для каждого проекта: `.nexusflow/commands/`
...
nexusflow включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
