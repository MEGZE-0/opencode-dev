---
title: TUI
description: Using the NexusFlow terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NexusFlow provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running NexusFlow starts the TUI for the current directory.
...
When using the NexusFlow TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to NexusFlow. Allows you to select from available providers and add their API keys.
...
Exit NexusFlow. _Aliases_: `/quit`, `/q`
...
This is separate from `nexusflow.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long NexusFlow waits after the leader key. Defaults to `2000`.
...
Use `NEXUSFLOW_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `nexusflow.default`.
