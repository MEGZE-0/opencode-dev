---
title: Server
description: Interact with nexusflow server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `nexusflow serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an nexusflow client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `nexusflow.local` |
...
Set `NEXUSFLOW_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `nexusflow`, or set `NEXUSFLOW_SERVER_USERNAME` to override it. This applies to both `nexusflow serve` and `nexusflow web`.
...
When you run `nexusflow` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the nexusflow server to interact with nexusflow programmatically.
:::

This architecture lets nexusflow support multiple clients and allows you to interact with nexusflow programmatically.

You can run `nexusflow serve` to start a standalone server. If you have the
nexusflow TUI running, `nexusflow serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the NexusFlow [IDE](/docs/ide) plugins.
...
The nexusflow server exposes the following APIs.
