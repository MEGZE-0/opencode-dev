...
Erstellen Sie `.nexusflow/commands/test.md`:

```md title=".nexusflow/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die NexusFlow-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem NexusFlow [config](/docs/config):

```json title="nexusflow.jsonc" {4-12}
{
...
- Global: `~/.config/nexusflow/commands/`
- Pro Projekt: `.nexusflow/commands/`

```markdown title="~/.config/nexusflow/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".nexusflow/commands/component.md"
---
...
```md title=".nexusflow/commands/create-file.md"
---
...
```md title=".nexusflow/commands/analyze-coverage.md"
---
...
```md title=".nexusflow/commands/review-changes.md"
---
...
```md title=".nexusflow/commands/review-component.md"
---
...
NexusFlow enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
