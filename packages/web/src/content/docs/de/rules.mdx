---
title: Regeln
description: Lege eigene Anweisungen fuer nexusflow fest.
---

Du kannst nexusflow ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in nexusflow den Befehl `/init` aus.
...
So kann nexusflow sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht nexusflow sie zu erweitern.
...
nexusflow kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/nexusflow/AGENTS.md` speichern.
Diese gelten in allen nexusflow-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt NexusFlow dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/nexusflow/AGENTS.md` existiert)
...
Beim Start sucht nexusflow in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/nexusflow/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `nexusflow.json` oder in `~/.config/nexusflow/nexusflow.json` angeben.
...
nexusflow parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit nexusflow.json

Empfohlen ist das `instructions`-Feld in der `nexusflow.json`:
...
Du kannst nexusflow explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `nexusflow.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
