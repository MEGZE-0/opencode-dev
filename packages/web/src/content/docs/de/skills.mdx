...
Agent Skills erlauben NexusFlow, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
NexusFlow sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft NexusFlow vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.nexusflow/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.nexusflow/skills/git-release/SKILL.md` zum Beispiel so:
...
NexusFlow listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `nexusflow.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `nexusflow.json`):
