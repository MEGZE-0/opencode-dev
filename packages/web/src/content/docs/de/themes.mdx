...
Mit NexusFlow kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet NexusFlow das `nexusflow`-Theme.
...
NexusFlow liefert mehrere eingebaute Themes mit.
...
- NexusFlow optisch ans Terminal anpassen wollen
...
NexusFlow unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/nexusflow/themes/*.json` oder `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.nexusflow/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.nexusflow/themes/*.json`
