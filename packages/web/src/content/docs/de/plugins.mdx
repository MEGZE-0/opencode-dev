---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um NexusFlow zu erweitern.
---

Mit Plugins können Sie NexusFlow erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von NexusFlow zu ändern.
...
- `.nexusflow/plugins/` – Plugins auf Projektebene
- `~/.config/nexusflow/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/nexusflow/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/nexusflow/nexusflow.json`)
2. Projektkonfiguration (`nexusflow.json`)
3. Globales Plugin-Verzeichnis (`~/.config/nexusflow/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.nexusflow/plugins/`)
...
NexusFlow führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein NexusFlow-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie NexusFlow erweitern können.
...
:::note
Wenn Sie die NexusFlow-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass NexusFlow `.env`-Dateien liest:
...
Plugins können NexusFlow auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das NexusFlow aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für NexusFlow zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://nexusflow.ai/docs/sdk).
