...
Wenn NexusFlow Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `nexusflow --log-level DEBUG`.

---

## Speicher

nexusflow speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
NexusFlow Desktop startet im Hintergrund einen lokalen NexusFlow-Server (`nexusflow-cli`-Sidecar).
...
- Nur macOS: `NexusFlow`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/nexusflow/nexusflow.jsonc` (oder `~/.config/nexusflow/nexusflow.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/nexusflow/nexusflow.jsonc`
...
NexusFlow kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit NexusFlow ihn neu aufbauen kann.

1. Beende NexusFlow Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/nexusflow`
- **Linux**: loesche `~/.cache/nexusflow` (oder fuehre aus: `rm -rf ~/.cache/nexusflow`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\nexusflow`

3. Starte NexusFlow Desktop neu.

---

### Server-Verbindungsprobleme beheben

NexusFlow Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `nexusflow.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `NEXUSFLOW_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `NEXUSFLOW_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt NexusFlow Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit NexusFlow-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

NexusFlow Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer NexusFlow in den OS-Einstellungen aktiviert sind, und
...
1. Beende NexusFlow Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von NexusFlow Desktop):

- `nexusflow.settings.dat` (desktop default server URL)
- `nexusflow.global.dat` und `nexusflow.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit NexusFlow hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `nexusflow models` aus.
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. nexusflow installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
2. Starte nexusflow neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt nexusflow, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
nexusflow erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
