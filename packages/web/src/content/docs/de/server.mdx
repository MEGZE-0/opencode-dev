---
title: Server
description: Nutze den nexusflow-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `nexusflow serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein nexusflow-Client nutzen kann.
...
Setze `NEXUSFLOW_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `nexusflow`, kann aber mit `NEXUSFLOW_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `nexusflow serve` und `nexusflow web`.
...
Wenn du `nexusflow` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den nexusflow-Server, um nexusflow programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `nexusflow serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `nexusflow serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die NexusFlow-[IDE](/docs/ide)-Plugins.
...
Der nexusflow-Server stellt folgende APIs bereit.
