...
You can add external tools to NexusFlow using the _Model Context Protocol_, or MCP. NexusFlow supports both local and remote servers.
...
You can define MCP servers in your [NexusFlow Config](https://nexusflow.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/nexusflow` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
NexusFlow automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, NexusFlow will:
...
If the server requires authentication, NexusFlow will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `nexusflow mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, NexusFlow will store the tokens securely in `~/.local/share/nexusflow/mcp-auth.json`.
...
Your MCPs are available as tools in NexusFlow, alongside built-in tools. So you can manage them through the NexusFlow config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect NexusFlow to your Sentry account.
