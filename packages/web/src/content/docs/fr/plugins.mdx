---
title: Plugins
description: Écrivez vos propres plugins pour étendre NexusFlow.
---

Les plugins vous permettent d'étendre NexusFlow en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de NexusFlow.
...
- `.nexusflow/plugins/` - Plugins au niveau du projet
- `~/.config/nexusflow/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/nexusflow/node_modules/`.
...
1. Configuration globale (`~/.config/nexusflow/nexusflow.json`)
2. Configuration du projet (`nexusflow.json`)
3. Répertoire global des plugins (`~/.config/nexusflow/plugins/`)
4. Répertoire des plugins du projet (`.nexusflow/plugins/`)
...
NexusFlow exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK nexusflow pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre nexusflow.
...
:::note
Si vous utilisez l'application de bureau NexusFlow, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez nexusflow de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à nexusflow :
...
L'assistant `tool` crée un outil personnalisé que nexusflow peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour nexusflow aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://nexusflow.ai/docs/sdk) pour plus de détails.
