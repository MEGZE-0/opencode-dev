...
Avec NexusFlow, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, NexusFlow utilise notre propre thème `nexusflow`.
...
NexusFlow est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que NexusFlow corresponde à l'apparence de leur terminal
...
NexusFlow prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/nexusflow/themes/*.json` ou `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.nexusflow/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.nexusflow/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/nexusflow/themes
vim ~/.config/nexusflow/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .nexusflow/themes
vim .nexusflow/themes/my-theme.json
```
...
{
  "$schema": "https://nexusflow.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
