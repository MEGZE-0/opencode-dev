---
title: Server
description: Komunicirajte s nexusflow serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `nexusflow serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji nexusflow klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `nexusflow.local` |
...
Postavite `NEXUSFLOW_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `nexusflow`, ili postavite `NEXUSFLOW_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `nexusflow serve` i za `nexusflow web`.
...
Kada pokrenete `nexusflow`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite nexusflow server za programsku interakciju sa nexusflow-om.
:::

Ova arhitektura omogucava nexusflow podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `nexusflow serve` da startate standalone server. Ako je nexusflow TUI vec pokrenut, `nexusflow serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste NexusFlow [IDE](/docs/ide) pluginovi.
...
nexusflow server izlaže sljedece API-je.
