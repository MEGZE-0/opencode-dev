...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. NexusFlow dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u NexusFlow.
...
:::note
Ovaj alat je dostupan samo kada je `NEXUSFLOW_EXPERIMENTAL_LSP_TOOL=true` (ili `NEXUSFLOW_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz NexusFlow provajdera ili kada je varijabla `NEXUSFLOW_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju NexusFlow:
