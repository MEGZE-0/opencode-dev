---
title: Dodaci
description: Napišite vlastite dodatke za proširenje NexusFlow.
---

Dodaci vam omogućavaju da proširite NexusFlow spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja NexusFlow.
...
- `.nexusflow/plugins/` - Dodaci na nivou projekta
- `~/.config/nexusflow/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/nexusflow/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/nexusflow/nexusflow.json`)
2. Konfiguracija projekta (`nexusflow.json`)
3. Globalni direktorij dodataka (`~/.config/nexusflow/plugins/`)
4. Direktorij dodataka projekta (`.nexusflow/plugins/`)
...
NexusFlow pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje NexusFlow.
...
Spriječite nexusflow da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u nexusflow:
...
Pomoćnik `tool` kreira prilagođeni alat koji nexusflow može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za nexusflow zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://nexusflow.ai/docs/sdk) za detalje.
