...
U NexusFlow mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, NexusFlow koristi nasu `nexusflow` temu.
...
NexusFlow dolazi sa vise ugradenih tema.
...
- Zele da NexusFlow odgovara izgledu njihovog terminala
...
NexusFlow podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/nexusflow/themes/*.json` ili `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.nexusflow/themes/*.json`
4. **Trenutni radni direktorij** - `./.nexusflow/themes/*.json`
