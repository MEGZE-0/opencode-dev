...
Postoje dvije vrste agenata u NexusFlow: primarni agenti i podagenti.
...
NexusFlow dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
NexusFlow dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

NexusFlow dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u NexusFlow-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `nexusflow.json`:
...
- Globalno: `~/.config/nexusflow/agents/`
- Po projektu: `.nexusflow/agents/`
...
Ako temperatura nije navedena, NexusFlow koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu NexusFlow konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj NexusFlow konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [NexusFlow Zen](/docs/zen), koristili biste `nexusflow/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `nexusflow models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/nexusflow).
:::
