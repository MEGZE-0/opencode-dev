---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u NexusFlow.
---

import config from "../../../../config.mjs"
export const console = config.console

NexusFlow koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj NexusFlow konfiguraciji.
...
u `~/.local/share/nexusflow/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem NexusFlow
config.
...
NexusFlow Zen je lista modela koje je obezbedio NexusFlow tim koji su bili
testirano i potvrđeno da dobro radi sa NexusFlow. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa NexusFlow Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite nexusflow i idite na [nexusflow.ai/auth](https://nexusflow.ai/auth).
...
Radi kao i svaki drugi provajder u NexusFlow i potpuno je opcionalan za korištenje.

---

## NexusFlow Go

NexusFlow Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža NexusFlow tim i koji su testirani i verificirani da dobro rade s NexusFlow-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `NexusFlow Go` i idite na [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Da biste koristili Amazon Bedrock s NexusFlow:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete nexusflow:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `nexusflow.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u NexusFlow.
:::
...
Možete konfigurirati nexusflow za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi nexusflow ispravno radio.
   :::
...
Također možete dodati modele kroz svoju nexusflow konfiguraciju.
...
da pohrani token u nexusflow auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
NexusFlow koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate NexusFlow
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`nexusflow.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth).

##### Konfiguracija

Prilagodite putem `nexusflow.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s nexusflow:
...
Za korištenje Google Vertex AI s NexusFlow:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće nexusflow.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem nexusflowa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session) da automatski prijavite svaki NexusFlow razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki NexusFlow razgovor naveden kao zasebna sesija.
...
Možete konfigurirati nexusflow za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati nexusflow za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati nexusflow za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za NexusFlow. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/nexusflow) za detalje.
:::
...
Da biste koristili Ollama Cloud s NexusFlow:
...
3. Kopirajte API ključ za korištenje u NexusFlow.
...
6. **Važno**: Prije upotrebe modela oblaka u NexusFlow, morate lokalno povući informacije o modelu:
...
NexusFlow Zen je lista testiranih i verifikovanih modela koju je obezbedio NexusFlow tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>NexusFlow Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **NexusFlow Zen**.

```txt
   /connect
```

3. Unesite svoj NexusFlow API ključ.
...
Također možete dodati dodatne modele putem vaše nexusflow konfiguracije.
...
5. Također ih možete prilagoditi putem vaše nexusflow konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju nexusflow konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s nexusflow-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `nexusflow.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju NexusFlow da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `nexusflow auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite NexusFlow konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj nexusflow konfiguraciji.
