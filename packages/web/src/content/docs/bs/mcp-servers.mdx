...
Možete dodati vanjske alate u NexusFlow koristeći _Model Context Protocol_, ili MCP. NexusFlow podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [NexusFlow Config](https://nexusflow.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/nexusflow`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
NexusFlow automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, NexusFlow će:
...
Ako server zahtijeva autentifikaciju, NexusFlow će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `nexusflow mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, NexusFlow sigurno čuva tokene u `~/.local/share/nexusflow/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u NexusFlow, zajedno s ugrađenim alatima. Možete njima upravljati kroz NexusFlow konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete NexusFlow sa vašim Sentry nalogom.
