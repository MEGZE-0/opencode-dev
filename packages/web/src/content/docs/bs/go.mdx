...
NexusFlow Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u NexusFlow-u. Pretplatite se na NexusFlow Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili NexusFlow.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa NexusFlow-om.
:::
...
NexusFlow Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

NexusFlow Go radi kao bilo koji drugi provajder u NexusFlow-u.

1. Prijavite se na **<a href={console}>NexusFlow Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `NexusFlow Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na NexusFlow Go.
:::
...
NexusFlow Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj NexusFlow konfiguraciji
koristi format `nexusflow-go/<model-id>`. Na primjer, za Kimi K2.6, koristili biste
`nexusflow-go/kimi-k2.6` u svojoj konfiguraciji.
...
Napravili smo NexusFlow Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz NexusFlow koristite i bilo kojeg drugog provajdera.
