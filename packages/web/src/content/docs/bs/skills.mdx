...
Agent skills omogucavaju NexusFlow da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
NexusFlow pretrazuje ove lokacije:

- Konfiguracija projekta: `.nexusflow/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, NexusFlow ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.nexusflow/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.nexusflow/skills/git-release/SKILL.md` ovako:
...
NexusFlow navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `nexusflow.json`:
...
**Za ugradene agente** (u `nexusflow.json`):
