---
title: LSP serveri
description: NexusFlow se integriše sa vašim LSP serverima.
---

NexusFlow se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

NexusFlow dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `NEXUSFLOW_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i nexusflow otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj nexusflow konfiguraciji.
