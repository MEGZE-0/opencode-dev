---
title: Konfiguracija
description: Korištenje NexusFlow JSON konfiguracije.
---

Možete konfigurirati NexusFlow koristeći JSON konfiguracijski fajl.

---

## Format

NexusFlow podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/nexusflow`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/nexusflow/nexusflow.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`NEXUSFLOW_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`nexusflow.json` u projektu) - postavke specifične za projekat
5. **`.nexusflow` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`NEXUSFLOW_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.nexusflow` i `~/.config/nexusflow` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/nexusflow` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu NexusFlow konfiguraciju u `~/.config/nexusflow/nexusflow.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/nexusflow/tui.json`.
...
Dodajte `nexusflow.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se NexusFlow pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NEXUSFLOW_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NEXUSFLOW_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.nexusflow` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.nexusflow`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`nexusflow.ai/config.json`**](https://nexusflow.ai/config.json).

TUI konfiguracija koristi [**`nexusflow.ai/tui.json`**](https://nexusflow.ai/tui.json).
...
Koristite `NEXUSFLOW_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `nexusflow.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `nexusflow serve` i `nexusflow web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš NexusFlow server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `nexusflow.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj NexusFlow konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, NexusFlow pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/nexusflow/agents/` ili `.nexusflow/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, NexusFlow će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`nexusflow run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/nexusflow/commands/` ili `.nexusflow/commands/`. [Saznajte više ovdje](/docs/commands).
...
NexusFlow će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, NexusFlow **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju NexusFlow sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.nexusflow/plugins/` ili `~/.config/nexusflow/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite NexusFlow da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
