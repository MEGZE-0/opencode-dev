...
nexusflow ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak nexusflow kendi `nexusflow` temasını kullanır.
...
nexusflow birden fazla yerleşik temayla gelir.
...
- nexusflow'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
nexusflow, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/nexusflow/themes/*.json` veya `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.nexusflow/themes/*.json`
4. **Geçerli çalışma dizini** - `./.nexusflow/themes/*.json`
