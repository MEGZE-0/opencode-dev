---
title: Sağlayıcılar
description: nexusflow'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

nexusflow, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı nexusflow yapılandırmanızda yapılandırın.
...
`~/.local/share/nexusflow/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları nexusflow'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
NexusFlow Zen, nexusflow ekibi tarafından sağlanan modellerin bir listesidir.
nexusflow ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz NexusFlow Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, nexusflow'u seçin ve [nexusflow.ai/auth](https://nexusflow.ai/auth)'ye gidin.
...
nexusflow'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## NexusFlow Go

NexusFlow Go, nexusflow ile iyi çalıştığı test edilmiş ve doğrulanmış, nexusflow ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `NexusFlow Go`'yu seçin ve [nexusflow.ai/auth](https://nexusflow.ai/zen) adresine gidin.
...
Amazon Bedrock'u nexusflow ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   nexusflow'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `nexusflow.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi nexusflow'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
nexusflow'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   nexusflow'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
nexusflow, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. nexusflow'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`nexusflow.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth) ana sayfasında.

##### Yapılandırma

`nexusflow.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi nexusflow'la kullanmak için:
...
Google Vertex AI'yi nexusflow ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları nexusflow çalıştırılırken ayarlayın.
...
Helicone'un nexusflow aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her nexusflow görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her nexusflow görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
nexusflow'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla nexusflow'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla nexusflow'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini nexusflow için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/nexusflow)'a bakın.
:::
...
Ollama Cloud'u nexusflow ile kullanmak için:
...
3. nexusflow'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: nexusflow'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
NexusFlow Zen, nexusflow ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>NexusFlow Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **NexusFlow Zen**'i arayın.

   ```txt
   /connect
   ```

3. nexusflow API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   nexusflow yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca nexusflow yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri nexusflow yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı nexusflow ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `nexusflow.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları nexusflow'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `nexusflow auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için nexusflow yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, nexusflow yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
