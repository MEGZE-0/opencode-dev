...
description: NexusFlow, LSP sunucularınızla bütünleşir.
---

NexusFlow, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

NexusFlow, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`NEXUSFLOW_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve nexusflow bir dosyayı açtığında:
...
LSP sunucularını, nexusflow yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
