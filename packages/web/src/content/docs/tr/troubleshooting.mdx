...
nexusflow ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\nexusflow\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `nexusflow --log-level DEBUG`.

---

## Depolama

nexusflow, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
nexusflow Desktop arka planda yerel bir nexusflow sunucusu (`nexusflow-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `nexusflow` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/nexusflow/nexusflow.jsonc` (veya `~/.config/nexusflow/nexusflow.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/nexusflow/nexusflow.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexusflow\nexusflow.jsonc` yapistirin
...
nexusflow diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/nexusflow/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexusflow\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), nexusflow'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. nexusflow Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/nexusflow`
- **Linux**: `~/.cache/nexusflow` dizinini silin (veya `rm -rf ~/.cache/nexusflow` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\nexusflow` yapistirin

3. nexusflow Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

nexusflow Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`nexusflow.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `NEXUSFLOW_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `NEXUSFLOW_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta nexusflow Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, nexusflow ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

nexusflow Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda nexusflow icin bildirimler etkinse
...
1. nexusflow Desktop'u kapatin
2. Su dosyalari bulun ve silin (nexusflow Desktop uygulama veri dizininde yer alirlar):

- `nexusflow.settings.dat` (desktop varsayilan sunucu URL'si)
- `nexusflow.global.dat` ve `nexusflow.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
nexusflow ile ilgili bir sorun yasiyorsaniz:
...
Erisiminiz olan modelleri gormek icin `nexusflow models` calistirin.
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. nexusflow, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin nexusflow'u yeniden baslatin

Bu, nexusflow'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
nexusflow, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
