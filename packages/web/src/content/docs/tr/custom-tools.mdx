---
title: Özel Araçlar
description: LLM'un nexusflow'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. nexusflow'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.nexusflow/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/nexusflow/tools/` içine yerleştirerek.
