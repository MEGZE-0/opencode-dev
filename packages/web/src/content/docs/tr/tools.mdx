...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. nexusflow yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
nexusflow içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `NEXUSFLOW_EXPERIMENTAL_LSP_TOOL=true` (veya `NEXUSFLOW_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca nexusflow provider kullanılırken veya `NEXUSFLOW_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

nexusflow başlatırken etkinleştirmek için:
