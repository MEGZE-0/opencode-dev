...
Ajan becerileri, nexusflow'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
nexusflow su konumlari tarar:

- Proje konfigurasyonu: `.nexusflow/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
Proje yerel yollarinda nexusflow, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.nexusflow/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.nexusflow/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
nexusflow, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`nexusflow.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`nexusflow.json` icinde):
