...
NexusFlow Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, NexusFlow'daki diğer sağlayıcılar gibi çalışır. NexusFlow Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve NexusFlow'u kullanmak için bunu kullanmanıza gerek yoktur.
...
NexusFlow Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

NexusFlow Go, NexusFlow'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>NexusFlow Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `NexusFlow Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye NexusFlow Go'ya abone olabilir.
:::
...
NexusFlow Go aşağıdaki limitleri içerir:
...
NexusFlow yapılandırmanızdaki [model id](/docs/config/#models) formatı `nexusflow-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `nexusflow-go/kimi-k2.6` kullanmalısınız.
...
NexusFlow Go'yu şu amaçlarla oluşturduk:
...
4. NexusFlow ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
