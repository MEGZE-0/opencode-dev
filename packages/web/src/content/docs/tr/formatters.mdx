...
description: nexusflow dile özgü biçimlendiriciler kullanır.
---

nexusflow, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

nexusflow, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, nexusflow bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

nexusflow bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri nexusflow yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
