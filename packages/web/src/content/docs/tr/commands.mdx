...
`.nexusflow/commands/test.md` oluştur:
...
nexusflow işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

nexusflow [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/nexusflow/commands/`
- Proje başına: `.nexusflow/commands/`
...
nexusflow, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
