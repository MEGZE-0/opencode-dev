---
title: Eklentiler
description: NexusFlow'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek NexusFlow'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya NexusFlow'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.nexusflow/plugins/` - Proje düzeyinde eklentiler
- `~/.config/nexusflow/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/nexusflow/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/nexusflow/nexusflow.json`)
2. Project config (`nexusflow.json`)
3. Global eklenti dizini (`~/.config/nexusflow/plugins/`)
4. Proje eklenti dizini (`.nexusflow/plugins/`)
...
NexusFlow bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir NexusFlow SDK istemcisi.
...
NexusFlow'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
NexusFlow masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
NexusFlow'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca NexusFlow'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, NexusFlow'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra NexusFlow'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://nexusflow.ai/docs/sdk) bakın.
