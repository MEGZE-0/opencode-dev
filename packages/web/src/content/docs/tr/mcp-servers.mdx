...
_Model Bağlam Protokolü_ veya MCP kullanarak nexusflow'a harici araçlar ekleyebilirsiniz. nexusflow hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [nexusflow Config](https://nexusflow.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/nexusflow` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
nexusflow, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde nexusflow şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa nexusflow, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `nexusflow mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından nexusflow, belirteçleri `~/.local/share/nexusflow/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra nexusflow'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi nexusflow yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve nexusflow'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
