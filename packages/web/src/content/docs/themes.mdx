...
With NexusFlow you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, NexusFlow uses our own `nexusflow` theme.
...
NexusFlow comes with several built-in themes.
...
- Want NexusFlow to match their terminal's appearance
...
NexusFlow supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/nexusflow/themes/*.json` or `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.nexusflow/themes/*.json`
4. **Current working directory** - Defined in `./.nexusflow/themes/*.json`
