---
title: Servidor
description: Interactuar con el servidor nexusflow a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `nexusflow serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente nexusflow puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `nexusflow.local` |
...
Configure `NEXUSFLOW_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `nexusflow`, o configure `NEXUSFLOW_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `nexusflow serve` como a `nexusflow web`.
...
Cuando ejecuta `nexusflow`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor nexusflow para interactuar con nexusflow mediante programación.
:::

Esta arquitectura permite que nexusflow admita múltiples clientes y le permite interactuar con nexusflow mediante programación.

Puede ejecutar `nexusflow serve` para iniciar un servidor independiente. Si tienes el
nexusflow TUI ejecutándose, `nexusflow serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos NexusFlow [IDE](/docs/ide).
...
El servidor nexusflow expone las siguientes API.
