...
Las habilidades del agente permiten a NexusFlow descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
NexusFlow busca estas ubicaciones:

- Configuración del proyecto: `.nexusflow/skills/<name>/SKILL.md`
- Configuración global: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, NexusFlow sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.nexusflow/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexusflow/skills/git-release/SKILL.md` así:
...
NexusFlow enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `nexusflow.json`:
...
**Para agentes integrados** (en `nexusflow.json`):
