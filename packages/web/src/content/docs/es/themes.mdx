...
Con NexusFlow puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, NexusFlow usa nuestro propio tema `nexusflow`.
...
NexusFlow viene con varios temas integrados.
...
- Quiere que NexusFlow coincida con la apariencia de su terminal
...
NexusFlow admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/nexusflow/themes/*.json` o `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.nexusflow/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.nexusflow/themes/*.json`
