---
title: Complementos
description: Escriba sus propios complementos para extender NexusFlow.
---

Los complementos le permiten extender NexusFlow al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de NexusFlow.
...
- `.nexusflow/plugins/` - Complementos a nivel de proyecto
- `~/.config/nexusflow/plugins/` - Complementos globales
...
```json title="nexusflow.json"
{
  ...
  "plugin": ["nexusflow-helicone-session", "nexusflow-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/nexusflow/node_modules/`.
...
1. Configuración global (`~/.config/nexusflow/nexusflow.json`)
2. Configuración del proyecto (`nexusflow.json`)
3. Directorio global de complementos (`~/.config/nexusflow/plugins/`)
4. Directorio de complementos del proyecto (`.nexusflow/plugins/`)
...
```json title=".nexusflow/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
NexusFlow ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".nexusflow/plugins/my-plugin.ts"
...
```js title=".nexusflow/plugins/example.js"
...
- `client`: Un cliente SDK nexusflow para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar nexusflow.
...
```js title=".nexusflow/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "nexusflow"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio NexusFlow, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que nexusflow lea archivos `.env`:

```javascript title=".nexusflow/plugins/env-protection.js"
...
```javascript title=".nexusflow/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a nexusflow:

```ts title=".nexusflow/plugins/custom-tools.ts"
import { type Plugin, tool } from "@nexusflow/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que nexusflow puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para nexusflow junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://nexusflow.ai/docs/sdk) para obtener más detalles.
...
```ts title=".nexusflow/plugins/compaction.ts"
import type { Plugin } from "@nexusflow/plugin"
...
```ts title=".nexusflow/plugins/custom-compaction.ts"
import type { Plugin } from "@nexusflow/plugin"
