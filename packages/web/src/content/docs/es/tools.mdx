...
Las herramientas permiten que LLM realice acciones en su código base. NexusFlow viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en NexusFlow.
...
:::note
Esta herramienta solo está disponible cuando `NEXUSFLOW_EXPERIMENTAL_LSP_TOOL=true` (o `NEXUSFLOW_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor NexusFlow o cuando la variable de entorno `NEXUSFLOW_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar NexusFlow:

```bash
NEXUSFLOW_ENABLE_EXA=1 nexusflow
```
