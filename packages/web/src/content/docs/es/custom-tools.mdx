...
description: Cree herramientas que LLM pueda llamar en nexusflow.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de nexusflow como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.nexusflow/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/nexusflow/tools/`.
...
```ts title=".nexusflow/tools/database.ts" {1}
...
```ts title=".nexusflow/tools/math.ts"
import { tool } from "@nexusflow/plugin"
...
```ts title=".nexusflow/tools/bash.ts"
import { tool } from "@nexusflow/plugin"
...
```ts title=".nexusflow/tools/project.ts" {8}
import { tool } from "@nexusflow/plugin"
...
```python title=".nexusflow/tools/add.py"
import sys
...
```ts title=".nexusflow/tools/python-add.ts" {10}
import { tool } from "@nexusflow/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".nexusflow/tools/add.py")
