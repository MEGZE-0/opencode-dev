---
title: Proveedores
description: Usando cualquier proveedor LLM en NexusFlow.
---

import config from "../../../../config.mjs"
export const console = config.console

NexusFlow usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración NexusFlow.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu NexusFlow
configuración.
...
NexusFlow Zen es una lista de modelos proporcionados por el equipo NexusFlow que han sido
probado y verificado para funcionar bien con NexusFlow. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con NexusFlow Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione nexusflow y diríjase a [nexusflow.ai/auth](https://nexusflow.ai/auth).
...
Funciona como cualquier otro proveedor en NexusFlow y su uso es completamente opcional.

---

## NexusFlow Go

NexusFlow Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de NexusFlow que han sido
probado y verificado para funcionar bien con NexusFlow.

1. Ejecute el comando `/connect` en TUI, seleccione `NexusFlow Go` y diríjase a [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Para usar Amazon Bedrock con NexusFlow:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta nexusflow:
   ...
   Para una configuración persistente o específica del proyecto, utilice `nexusflow.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en NexusFlow no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar nexusflow para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que nexusflow funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración nexusflow.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `NexusFlow`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación nexusflow.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
NexusFlow utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear NexusFlow
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `nexusflow.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth).

##### Configuración

Personalizar a través de `nexusflow.json`:
...
Para utilizar su suscripción GitHub Copilot con nexusflow:
...
Para utilizar Google Vertex AI con NexusFlow:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta nexusflow.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de nexusflow, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session) para registrar automáticamente cada conversación NexusFlow como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación NexusFlow enumerada como una sesión separada.
...
Puede configurar nexusflow para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar nexusflow para usar modelos locales a través de LM Studio.
...
Puedes configurar nexusflow para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para NexusFlow. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/nexusflow) para obtener más detalles.
:::
...
Para usar Ollama Cloud con NexusFlow:
...
3. Copie la clave API para usarla en NexusFlow.
...
6. **Importante**: Antes de usar modelos en la nube en NexusFlow, debe obtener la información del modelo localmente:
...
NexusFlow Zen es una lista de modelos probados y verificados proporcionada por el equipo NexusFlow. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>NexusFlow Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **NexusFlow Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave NexusFlow API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración nexusflow.
...
5. También puedes personalizarlos a través de tu configuración nexusflow. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con NexusFlow:
...
También puede personalizar modelos a través de su configuración nexusflow. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con nexusflow. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `nexusflow.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a NexusFlow comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `nexusflow auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración nexusflow y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración nexusflow.
