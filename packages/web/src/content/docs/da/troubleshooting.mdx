...
For at fejlfinde problemer med NexusFlow, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `nexusflow --log-level DEBUG`.

---

## Lagring

nexusflow gemmer sessionsdata og andre applikationsdata på disken på:
...
NexusFlow Desktop kører en lokal NexusFlow-server (`nexusflow-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `NexusFlow`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/nexusflow/nexusflow.jsonc` (eller `~/.config/nexusflow/nexusflow.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/nexusflow/nexusflow.jsonc`
...
NexusFlow kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så NexusFlow kan genopbygge den.

1. Afslut NexusFlow Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/nexusflow`
- **Linux**: slet `~/.cache/nexusflow` (eller kør `rm -rf ~/.cache/nexusflow`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\nexusflow`

3. Start NexusFlow Desktop på ny.

---

### Løs problemer med serverforbindelse

NexusFlow Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `nexusflow.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `NEXUSFLOW_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NEXUSFLOW_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver NexusFlow Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i NexusFlow.

---

### Meddelelser vises ikke

NexusFlow Desktop viser kun systemvarsler når:

- varsler er aktiveret for NexusFlow i dine OS-indstillinger, og
...
1. Afslut NexusFlow Desktop.
2. Find og slet disse filer (de findes i NexusFlow Desktop-appens datamappe):

- `nexusflow.settings.dat` (skrivebordsstandardserver URL)
- `nexusflow.global.dat` og `nexusflow.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med NexusFlow:
...
For at finde ud af hvilke modeller du har adgang til, kør `nexusflow models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. nexusflow installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start nexusflow på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge nexusflow til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
nexusflow vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
