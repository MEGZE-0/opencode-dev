...
Med NexusFlow kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger NexusFlow vårt eget `nexusflow`-tema.
...
NexusFlow kommer med flere innebygde temaer.
...
- Vil at NexusFlow skal matche terminalens utseende
...
NexusFlow støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/nexusflow/themes/*.json` eller `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.nexusflow/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.nexusflow/themes/*.json`
