...
Agentferdigheter lar NexusFlow oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
NexusFlow søker etter disse stedene:

- Prosjektkonfigurasjon: `.nexusflow/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
For prosjektlokale stier går NexusFlow opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.nexusflow/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.nexusflow/skills/git-release/SKILL.md` slik:
...
NexusFlow viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `nexusflow.json`:
...
**For innebygde agenter** (i `nexusflow.json`):
