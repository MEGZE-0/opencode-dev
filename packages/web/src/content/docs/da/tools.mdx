...
Her er alle de innebygde verktøyene som er tilgængelige i NexusFlow.
...
:::note
Dette verktøyet er bare tilgængelig når `NEXUSFLOW_EXPERIMENTAL_LSP_TOOL=true` (eller `NEXUSFLOW_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger NexusFlow-leverandøren eller når miljøvariabelen `NEXUSFLOW_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
