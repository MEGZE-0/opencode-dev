---
title: Plugins
description: Skriv dine egne plugins for at udvide NexusFlow.
---

Plugins giver dig mulighed for at udvide NexusFlow ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre NexusFlows standardadfærd.
...
- `.nexusflow/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/nexusflow/node_modules/`.
...
1. Global konfiguration (`~/.config/nexusflow/nexusflow.json`)
2. Projektkonfiguration (`nexusflow.json`)
3. Global plugin-mappe (`~/.config/nexusflow/plugins/`)
4. Projekt plugin bibliotek (`.nexusflow/plugins/`)
...
NexusFlow kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En nexusflow SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide nexusflow.
...
:::note
Hvis du bruger NexusFlow desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå nexusflow i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til nexusflow:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som nexusflow kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for nexusflow sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://nexusflow.ai/docs/sdk) for detaljer.
