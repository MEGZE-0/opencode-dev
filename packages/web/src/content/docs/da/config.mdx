---
title: Konfiguration
description: Ved at bruge NexusFlow JSON konfig.
---

Du kan konfigurere NexusFlow ved hjælp af en JSON-konfigurationsfil.

---

## Format

NexusFlow understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/nexusflow`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/nexusflow/nexusflow.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`NEXUSFLOW_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`nexusflow.json` i projekt) - projektspecifikke indstillinger
5. **`.nexusflow` mapper** - agent, kommandoer, plugins
6. **Inline config** (`NEXUSFLOW_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.nexusflow` og `~/.config/nexusflow` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/nexusflow`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale NexusFlow-konfiguration i `~/.config/nexusflow/nexusflow.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `nexusflow.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når NexusFlow starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `NEXUSFLOW_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `NEXUSFLOW_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.nexusflow` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.nexusflow` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`nexusflow.ai/config.json`**](https://nexusflow.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `nexusflow serve` og `nexusflow web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din NexusFlow-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `nexusflow.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din NexusFlow-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger NexusFlow at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din NexusFlow-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/nexusflow/agents/` eller `.nexusflow/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil NexusFlow falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`nexusflow run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/nexusflow/commands/` eller `.nexusflow/commands/`. [Learn more here](/docs/commands).
...
NexusFlow vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard nexusflow **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide NexusFlow med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.nexusflow/plugins/` eller `~/.config/nexusflow/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse NexusFlow til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
