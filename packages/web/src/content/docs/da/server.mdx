---
title: Server
description: Interager med nexusflow-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexusflow serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en nexusflow-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `nexusflow.local` |
...
Indstil `NEXUSFLOW_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `nexusflow`, eller indstil `NEXUSFLOW_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `nexusflow serve` og `nexusflow web`.
...
Når du kører `nexusflow` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug nexusflow-serveren til at interagere med nexusflow programmatisk.
:::

Denne arkitektur lader nexusflow understøtte flere klienter og lader dig interagere med nexusflow programmatisk.

Du kan køre `nexusflow serve` for at starte en selvstændig server. Hvis du har
nexusflow TUI kørende, vil `nexusflow serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af NexusFlow [IDE](/docs/ide) plugins.
...
NexusFlow-serveren viser følgende API'er.
