---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i NexusFlow.
---

import config from "../../../../config.mjs"
export const console = config.console

NexusFlow bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i NexusFlow-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/nexusflow/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i NexusFlow-konfigurationen.
...
NexusFlow Zen er en liste over modeller leveret af NexusFlow-teamet, som er blevet testet og verificeret til at fungere godt med NexusFlow. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med NexusFlow Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg nexusflow og gå til [nexusflow.ai/auth](https://nexusflow.ai/auth).
...
Det fungerer som alle andre udbydere i NexusFlow og er helt valgfrit at bruge.

---

## NexusFlow Go

NexusFlow Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af NexusFlow-teamet, som er testet og verificeret til at fungere godt med NexusFlow.

1. Kør kommandoen `/connect` i TUI, vælg `NexusFlow Go`, og gå til [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Sådan bruger du Amazon Bedrock med NexusFlow:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører nexusflow:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `nexusflow.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i NexusFlow understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere nexusflow til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at nexusflow skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem nexusflow-konfigurationen.
...
gemme tokenet i nexusflow auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
NexusFlow bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse NexusFlow
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`nexusflow.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `nexusflow.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med NexusFlow:
...
Sådan bruger du Google Vertex AI med NexusFlow:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører nexusflow.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem nexusflow, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session) plugin for automatisk at logge hver NexusFlow-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver NexusFlow-samtale opført som en separat session.
...
Du kan konfigurere nexusflow til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere nexusflow til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere nexusflow til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for NexusFlow. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/nexusflow) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med NexusFlow:
...
3. Kopier API-nøglen til brug i NexusFlow.
...
6. **Vigtigt**: Før du bruger skymodeller i NexusFlow, skal du hente modelinformationen lokalt:
...
NexusFlow Zen er en liste over testede og verificerede modeller leveret af NexusFlow-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>NexusFlow Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **NexusFlow Zen**.

   ```txt
   /connect
   ```

3. Indtast NexusFlow API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem nexusflow-konfigurationen.
...
5. Du kan også tilpasse dem gennem nexusflow-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med NexusFlow:
...
Du kan også tilpasse modeller gennem nexusflow-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med nexusflow. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `nexusflow.json`-filen i projektkataloget:
...
`limit`-felterne lader NexusFlow forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `nexusflow auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek nexusflow-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i nexusflow-konfigurationen.
