...
أنشئ الملف `.nexusflow/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات NexusFlow أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ NexusFlow:
...
- على مستوى النظام: `~/.config/nexusflow/commands/`
- لكل مشروع: `.nexusflow/commands/`
...
يتضمن nexusflow عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
