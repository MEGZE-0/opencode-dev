...
يمكنك إضافة أدوات خارجية إلى NexusFlow باستخدام _Model Context Protocol_، أو MCP. يدعم NexusFlow خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات NexusFlow](https://nexusflow.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/nexusflow` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى NexusFlow تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم NexusFlow بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك NexusFlow المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `nexusflow mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن NexusFlow الرموز بشكل آمن في `~/.local/share/nexusflow/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل NexusFlow إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات NexusFlow مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط NexusFlow بحساب Sentry الخاص بك.
