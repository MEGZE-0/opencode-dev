...
تتيح مهارات الوكيل لـ NexusFlow اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث NexusFlow في هذه المواقع:

- إعدادات المشروع: `.nexusflow/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج NexusFlow صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.nexusflow/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/nexusflow/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.nexusflow/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض NexusFlow المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `nexusflow.json`:
...
**للوكلاء المدمجين** (في `nexusflow.json`):
