...
مع NexusFlow يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم NexusFlow سمتنا `nexusflow`.
...
يأتي NexusFlow مع عدة سمات مدمجة.
...
- يريدون أن يطابق NexusFlow مظهر terminal لديهم
...
يدعم NexusFlow نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/nexusflow/themes/*.json` أو `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.nexusflow/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.nexusflow/themes/*.json`
