...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في nexusflow.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في nexusflow مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.nexusflow/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/nexusflow/tools/`.
