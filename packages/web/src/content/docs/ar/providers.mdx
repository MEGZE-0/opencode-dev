---
title: المزوّدون
description: استخدام أي موفّر LLM في NexusFlow.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم NexusFlow ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات NexusFlow.
...
في `~/.local/share/nexusflow/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات NexusFlow.
...
NexusFlow Zen هي قائمة نماذج يوفّرها فريق NexusFlow وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع NexusFlow. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع NexusFlow Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر nexusflow، ثم انتقل إلى [nexusflow.ai/auth](https://nexusflow.ai/auth).
...
يعمل مثل أي مزوّد آخر في NexusFlow واستخدامه اختياري بالكامل.

---

## NexusFlow Go

NexusFlow Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق NexusFlow، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع NexusFlow.

1. شغّل الأمر `/connect` في TUI، واختر `NexusFlow Go`، ثم انتقل إلى [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
لاستخدام Amazon Bedrock مع NexusFlow:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل nexusflow:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `nexusflow.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل NexusFlow غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين nexusflow لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل nexusflow بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات nexusflow.
...
تخزين الرمز في مساحة تخزين مصادقة nexusflow.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم NexusFlow نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر NexusFlow
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`nexusflow.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth).

##### التهيئة

خصّص عبر `nexusflow.json`:
...
لاستخدام اشتراك GitHub Copilot مع nexusflow:
...
لاستخدام Google Vertex AI مع NexusFlow:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر nexusflow، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session) لتسجيل كل محادثة NexusFlow تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة NexusFlow مدرجة كجلسة مستقلة.
...
يمكنك ضبط nexusflow لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط nexusflow لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط nexusflow لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع NexusFlow. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/nexusflow) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع NexusFlow:
...
3. انسخ مفتاح API لاستخدامه في NexusFlow.
...
6. **مهم**: قبل استخدام نماذج السحابة في NexusFlow، يجب سحب معلومات النموذج محليا:
...
NexusFlow Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق NexusFlow. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>NexusFlow Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **NexusFlow Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ NexusFlow.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات nexusflow.
...
5. يمكنك أيضا تخصيصها عبر إعدادات nexusflow. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات nexusflow. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع nexusflow. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `nexusflow.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ NexusFlow بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `nexusflow auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات nexusflow وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات nexusflow.
