...
Create `.nexusflow/commands/test.md`:
...
You can add custom commands through the NexusFlow config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your NexusFlow [config](/docs/config):
...
- Per-project: `.nexusflow/commands/`
...
nexusflow includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
