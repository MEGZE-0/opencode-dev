---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w nexusflow.
---

import config from "../../../../config.mjs"
export const console = config.console

nexusflow używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji nexusflow.
...
w `~/.local/share/nexusflow/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim nexusflow
config.
...
NexusFlow Zen to lista modeli dostarczonych przez zespół nexusflow, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z nexusflow. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od NexusFlow Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `NexusFlow Zen` i przejdź do [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Działa jak każdy inny dostawca w nexusflow i jest całkowicie opcjonalny w użyciu.

---

## NexusFlow Go

NexusFlow Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół nexusflow, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z nexusflow.

1. Uruchom polecenie `/connect` w TUI, wybierz `NexusFlow Go` i przejdź do [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Aby używać Amazon Bedrock z nexusflow:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania nexusflow:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `nexusflow.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w nexusflow nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować nexusflow do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod nexusflow działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji nexusflow.
...
nexusflow używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować nexusflow
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`nexusflow.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem nexusflow:
...
Aby używać Google Vertex AI z nexusflow:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania nexusflow.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą nexusflow, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session), aby automatycznie rejestrować każdą konwersację nexusflow jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja nexusflow będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować nexusflow tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować nexusflow tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować nexusflow tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem nexusflow. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/nexusflow).
:::
...
Aby korzystać z Ollama Cloud z nexusflow:
...
3. Skopiuj klucz API do użycia w nexusflow.
...
6. **Ważne**: Przed użyciem modeli chmurowych w nexusflow musisz pobrać informacje o modelu lokalnie:
...
NexusFlow Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół nexusflow. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>NexusFlow Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **NexusFlow Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API nexusflow.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji nexusflow.
...
5. Możesz je także dostosować za pomocą konfiguracji nexusflow. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z nexusflow:
...
Możesz także dostosować modele za pomocą konfiguracji nexusflow. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z nexusflow. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `nexusflow.json` w katalogu projektu:
...
Pola `limit` pozwalają nexusflow zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `nexusflow auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację nexusflow i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji nexusflow.
