...
Dzięki nexusflow możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie nexusflow używa naszego własnego motywu `nexusflow`.
...
nexusflow ma kilka wbudowanych motywów.
...
- Chcą, aby NexusFlow pasował do wyglądu ich terminala
...
nexusflow obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/nexusflow/themes/*.json` lub `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.nexusflow/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.nexusflow/themes/*.json`
