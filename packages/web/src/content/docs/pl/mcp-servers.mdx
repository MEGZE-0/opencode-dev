...
Możesz dodać dodatkowe informacje do nexusflow za pomocą _Model Context Protocol_, w skrócie MCP. nexusflow obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [nexusflow Config](https://nexusflow.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/nexusflow`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
nexusflow automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, nexusflow:
...
Jeśli serwer wymaga uwierzytelnienia, nexusflow poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `nexusflow mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji nexusflow bezpieczne przechowa tokeny w `~/.local/share/nexusflow/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w nexusflow, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie nexusflow, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć nexusflow z kontem Sentry.
