---
title: Serwery LSP
description: NexusFlow integruje się z serwerami LSP.
---

NexusFlow może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

NexusFlow posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `NEXUSFLOW_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i NexusFlow otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji NexusFlow.
