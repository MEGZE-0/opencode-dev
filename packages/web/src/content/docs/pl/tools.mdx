...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. NexusFlow posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w NexusFlow.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `NEXUSFLOW_EXPERIMENTAL_LSP_TOOL=true` (lub `NEXUSFLOW_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy NexusFlow lub gdy zmienna środowiskowa `NEXUSFLOW_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu NexusFlow:
