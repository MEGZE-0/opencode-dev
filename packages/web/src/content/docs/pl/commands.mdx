...
Utwórz `.nexusflow/commands/test.md`:
...
Możesz zdefiniować polecenia w `nexusflow.json` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji nexusflow:
...
- Globalnie: `~/.config/nexusflow/commands/`
- Na projekt: `.nexusflow/commands/`
...
nexusflow zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
