---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć nexusflow.
---

Wtyczki umożliwiające wyłączenie nexusflow poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania nexusflow.
...
- `.nexusflow/plugins/` - Wtyczki na poziomie projektu
- `~/.config/nexusflow/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/nexusflow/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/nexusflow/nexusflow.json`)
2. Konfiguracja projektu (`nexusflow.json`)
3. Globalny katalog wtyczek (`~/.config/nexusflow/plugins/`)
4. Katalog wtyczek projektu (`.nexusflow/plugins/`)
...
nexusflow uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z nexusflow do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do nexusflow.
...
:::note
Jeśli korzystasz z aplikacji komputerowej nexusflow, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj nexusflow czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do nexusflow:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod nexusflow. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://nexusflow.ai/docs/sdk).
