---
title: Konfiguracja
description: Ustawienia NexusFlow w pliku JSON.
---

Możesz dostosować NexusFlow za pomocą pliku konfiguracyjnego JSON.

---

## Format

NexusFlow obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "nexusflow"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/nexusflow`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/nexusflow/nexusflow.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `NEXUSFLOW_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`nexusflow.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.nexusflow`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `NEXUSFLOW_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.nexusflow` i `~/.config/nexusflow` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/nexusflow`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację NexusFlow w `~/.config/nexusflow/nexusflow.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/nexusflow/tui.json`.
...
Dodaj `nexusflow.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy NexusFlow się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `NEXUSFLOW_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `NEXUSFLOW_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.nexusflow` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.nexusflow`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`nexusflow.ai/config.json`**](https://nexusflow.ai/config.json).
...
Użyj `NEXUSFLOW_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `nexusflow.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `nexusflow serve` i `nexusflow web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer NexusFlow.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `nexusflow.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie NexusFlow wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/nexusflow/agents/` lub `.nexusflow/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, NexusFlow powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`nexusflow run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/nexusflow/commands/` lub `.nexusflow/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
NexusFlow może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy NexusFlow nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie NexusFlow **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają NexusFlow o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.nexusflow/plugins/` lub `~/.config/nexusflow/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie NexusFlow tylko do określonych dostawców.
