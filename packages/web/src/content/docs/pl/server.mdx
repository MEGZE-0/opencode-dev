---
title: Serwer
description: Interakcja z serwerem nexusflow za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `nexusflow serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient nexusflow.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `nexusflow.local` |
...
Ustaw `NEXUSFLOW_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `nexusflow` lub ustaw `NEXUSFLOW_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `nexusflow serve`, jak i `nexusflow web`.
...
Kiedy uruchomisz `nexusflow`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera nexusflow do programowej interakcji z nexusflow.
:::

Ta architektura umożliwia obsługę wielu klientów przez nexusflow i programową interakcję z kodem otwartym.

Możesz uruchomić `nexusflow serve`, aby uruchomić samodzielny serwer. Jeśli masz
nexusflow TUI działa, `nexusflow serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki nexusflow [IDE](/docs/ide).
...
Serwer nexusflow udostępnia następujące interfejsy API.
