...
NexusFlow Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w NexusFlow. Subskrybujesz NexusFlow Go i
...
NexusFlow Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

NexusFlow Go działa jak każdy inny dostawca w NexusFlow.

1. Logujesz się do **<a href={console}>NexusFlow Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `NexusFlow Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować NexusFlow Go.
:::
...
NexusFlow Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji NexusFlow
używa formatu `nexusflow-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`nexusflow-go/kimi-k2.6` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z NexusFlow.
