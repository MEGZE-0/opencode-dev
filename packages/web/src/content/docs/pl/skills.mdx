...
Umiejętności agenta pozwalają nexusflow odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
nexusflow przeszukuje te lokalizacje:

- Project config: `.nexusflow/skills/<name>/SKILL.md`
- Global config: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu nexusflow przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.nexusflow/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.nexusflow/skills/git-release/SKILL.md` w ten sposób:
...
nexusflow wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `nexusflow.json`:
...
**Dla agentów wbudowanych** (w `nexusflow.json`):
