...
Aby debugować problemy z nexusflow, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `nexusflow --log-level DEBUG`.

---

## Przechowywanie danych

nexusflow przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
nexusflow Desktop uruchamia w tle lokalny serwer nexusflow (sidecar `nexusflow-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `NexusFlow` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/nexusflow/nexusflow.jsonc` (lub `~/.config/nexusflow/nexusflow.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/nexusflow/nexusflow.jsonc`
...
nexusflow może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby nexusflow mógł ją odbudować.

1. Całkowicie zamknij nexusflow Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexusflow`
- **Linux**: usuń `~/.cache/nexusflow` (lub uruchom `rm -rf ~/.cache/nexusflow`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\nexusflow`

3. Uruchom ponownie nexusflow Desktop.

---

### Napraw problemy z połączeniem z serwerem

nexusflow Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `nexusflow.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `NEXUSFLOW_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `NEXUSFLOW_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows nexusflow Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami nexusflow.

---

### Brak powiadomień

nexusflow Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla nexusflow w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie nexusflow Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji nexusflow Desktop):

- `nexusflow.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `nexusflow.global.dat` i `nexusflow.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z nexusflow:
...
3. Upewnij się, że masz najnowszą wersję z `nexusflow upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `nexusflow models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. nexusflow dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod nexusflow, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to nexusflow do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
nexusflow wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
