---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i NexusFlow.
---

import config from "../../../../config.mjs"
export const console = config.console

NexusFlow bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i NexusFlow-konfigurasjonen.
...
i `~/.local/share/nexusflow/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i NexusFlow
konfig.
...
NexusFlow Zen er en liste over modeller levert av NexusFlow-teamet som har vært
testet og verifisert for å fungere godt med NexusFlow. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med NexusFlow Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `NexusFlow Zen` og gå til [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Det fungerer som alle andre leverandører i NexusFlow og er helt valgfritt å bruke.

---

## NexusFlow Go

NexusFlow Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av NexusFlow-teamet som har vært
testet og verifisert for å fungere godt med NexusFlow.

1. Kjør kommandoen `/connect` i TUI, velg `NexusFlow Go`, og gå til [nexusflow.ai/auth](https://nexusflow.ai/zen).
...
Slik bruker du Amazon Bedrock med NexusFlow:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører NexusFlow:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `nexusflow.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i nexusflow støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere nexusflow til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at NexusFlow skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom NexusFlow-konfigurasjonen.
...
for å lagre token i NexusFlow auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
NexusFlow bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse NexusFlow
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`nexusflow.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [nexusflow-gitlab-auth](https://www.npmjs.com/package/nexusflow-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `nexusflow.json`:
...
Slik bruker du GitHub Copilot-abonnementet med NexusFlow:
...
Slik bruker du Google Vertex AI med NexusFlow:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører NexusFlow.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom NexusFlow, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [nexusflow-helicone-session](https://github.com/H2Shami/nexusflow-helicone-session) plugin for automatisk å logge hver NexusFlow-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver NexusFlow-samtale oppført som en separat økt.
...
Du kan konfigurere NexusFlow for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere NexusFlow for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere NexusFlow for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for NexusFlow. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/nexusflow) for detaljer.
:::
...
Slik bruker du Ollama Cloud med NexusFlow:
...
3. Kopier API-nøkkelen for bruk i NexusFlow.
...
6. **Viktig**: Før du bruker skymodeller i NexusFlow, må du hente modellinformasjonen lokalt:
...
NexusFlow Zen er en liste over testede og verifiserte modeller levert av NexusFlow-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>NexusFlow Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **NexusFlow Zen**.

   ```txt
   /connect
   ```

3. Skriv inn NexusFlow API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom NexusFlow-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom NexusFlow-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med NexusFlow:
...
Du kan også tilpasse modeller gjennom NexusFlow-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med NexusFlow. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `nexusflow.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar NexusFlow forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `nexusflow auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk NexusFlow-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i NexusFlow-konfigurasjonen.
