---
title: LSP-servere
description: nexusflow integreres med LSP-serverne dine.
---

NexusFlow kan integreres med Language Server Protocol (LSP)-servere for å bruke diagnostikk som tilbakemelding til agenten.

---

## Innebygde servere

NexusFlow kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `NEXUSFLOW_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når LSP er aktivert og nexusflow åpner en fil, gjør den:
...
Du kan aktivere og tilpasse LSP-servere gjennom `lsp`-delen i NexusFlow-konfigurasjonen.
