---
title: Tjener
description: Samhandle med NexusFlow-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexusflow serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en NexusFlow-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `nexusflow.local` |
...
Sett `NEXUSFLOW_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `nexusflow` som standard, eller sett `NEXUSFLOW_SERVER_USERNAME` for å overstyre det. Dette gjelder både `nexusflow serve` og `nexusflow web`.
...
Når du kjører `nexusflow` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk NexusFlow-serveren til å samhandle med NexusFlow programmatisk.
:::

Denne arkitekturen lar NexusFlow støtte flere klienter og lar deg samhandle med NexusFlow programmatisk.

Du kan kjøre `nexusflow serve` for å starte en frittstående server. Hvis du har
NexusFlow TUI kjører, vil `nexusflow serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av NexusFlow [IDE](/docs/ide) plugins.
...
nexusflow-serveren viser følgende APIer.
