...
Du kan legge til eksterne verktøy i NexusFlow ved å bruke _Model Context Protocol_ eller MCP. NexusFlow støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [NexusFlow Config](https://nexusflow.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="nexusflow.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/nexusflow`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="nexusflow.jsonc" {15}
{
  "$schema": "https://nexusflow.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
NexusFlow håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil NexusFlow:
...
Hvis serveren krever autentisering, vil NexusFlow be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `nexusflow mcp auth <server-name>`.
...
```json title="nexusflow.json" {7-11}
{
  "$schema": "https://nexusflow.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
nexusflow mcp auth my-oauth-server
```
...
```bash
nexusflow mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil NexusFlow lagre tokens sikkert i `~/.local/share/nexusflow/mcp-auth.json`.
...
nexusflow mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i NexusFlow, sammen med innebygde verktøy. Så du kan administrere dem gjennom NexusFlow-konfigurasjonen som alle andre verktøy.
...
```json title="nexusflow.json" {11, 14-18}
{
  "$schema": "https://nexusflow.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
nexusflow mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble NexusFlow til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver NexusFlow-samtale oppført som en separat økt.
