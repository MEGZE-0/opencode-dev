...
Para depurar problemas com o nexusflow, comece verificando os logs e os dados locais que ele armazena no disco.
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `nexusflow --log-level DEBUG`.

---

## Armazenamento

O nexusflow armazena dados de sessão e outros dados do aplicativo no disco em:
...
O nexusflow Desktop executa um servidor nexusflow local (o sidecar `nexusflow-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `NexusFlow` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/nexusflow/nexusflow.jsonc` (ou `~/.config/nexusflow/nexusflow.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/nexusflow/nexusflow.jsonc`
...
O nexusflow também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o nexusflow possa reconstruí-lo.

1. Saia completamente do nexusflow Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/nexusflow`
- **Linux**: exclua `~/.cache/nexusflow` (ou execute `rm -rf ~/.cache/nexusflow`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\nexusflow`

3. Reinicie o nexusflow Desktop.

---

### Corrigindo problemas de conexão com o servidor

O nexusflow Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `nexusflow.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `NEXUSFLOW_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `NEXUSFLOW_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o nexusflow Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do nexusflow.

---

### Notificações não aparecendo

O nexusflow Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o nexusflow nas configurações do seu sistema operacional, e
...
1. Saia do nexusflow Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo nexusflow Desktop):

- `nexusflow.settings.dat` (URL do servidor padrão do desktop)
- `nexusflow.global.dat` e `nexusflow.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o nexusflow:
...
Para descobrir quais modelos você tem acesso, execute `nexusflow models`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O nexusflow instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
2. Reinicie o nexusflow para reinstalar os pacotes de provedores mais recentes

Isso forçará o nexusflow a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O nexusflow detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
