...
Com o nexusflow, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o nexusflow usa nosso próprio tema `nexusflow`.
...
O nexusflow vem com vários temas embutidos.
...
- Querem que o nexusflow corresponda à aparência do seu terminal
...
O nexusflow suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/nexusflow/themes/*.json` ou `$XDG_CONFIG_HOME/nexusflow/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.nexusflow/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.nexusflow/themes/*.json`
