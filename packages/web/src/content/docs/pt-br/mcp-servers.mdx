...
Você pode adicionar ferramentas externas ao nexusflow usando o _Model Context Protocol_, ou MCP. O nexusflow suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do nexusflow](https://nexusflow.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/nexusflow`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O nexusflow lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o nexusflow irá:
...
Se o servidor requer autenticação, o nexusflow solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `nexusflow mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o nexusflow armazenará os tokens de forma segura em `~/.local/share/nexusflow/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no nexusflow, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do nexusflow como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o nexusflow à sua conta do Sentry.
