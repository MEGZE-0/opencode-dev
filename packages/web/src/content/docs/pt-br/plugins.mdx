---
title: Plugins
description: Escreva seus próprios plugins para estender o nexusflow.
---

Plugins permitem que você estenda o nexusflow conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do nexusflow.
...
- `.nexusflow/plugins/` - Plugins em nível de projeto
- `~/.config/nexusflow/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/nexusflow/node_modules/`.
...
1. Configuração global (`~/.config/nexusflow/nexusflow.json`)
2. Configuração do projeto (`nexusflow.json`)
3. Diretório de plugins global (`~/.config/nexusflow/plugins/`)
4. Diretório de plugins do projeto (`.nexusflow/plugins/`)
...
O nexusflow executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do nexusflow para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o nexusflow.
...
:::note
Se você estiver usando o aplicativo desktop nexusflow, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o nexusflow de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao nexusflow:
...
O helper `tool` cria uma ferramenta personalizada que o nexusflow pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o nexusflow junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://nexusflow.ai/docs/sdk) para detalhes.
