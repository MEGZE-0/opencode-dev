...
As habilidades do agente permitem que o nexusflow descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O nexusflow pesquisa nesses locais:

- Configuração do projeto: `.nexusflow/skills/<name>/SKILL.md`
- Configuração global: `~/.config/nexusflow/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o nexusflow sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.nexusflow/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/nexusflow/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.nexusflow/skills/git-release/SKILL.md` assim:
...
O nexusflow lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `nexusflow.json`:
...
**Para agentes embutidos** (em `nexusflow.json`):
