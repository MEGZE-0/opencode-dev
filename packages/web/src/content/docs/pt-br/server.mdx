---
title: Servidor
description: Interaja com o servidor nexusflow via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `nexusflow serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente nexusflow pode usar.
...
```bash
nexusflow serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `nexusflow.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
nexusflow serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `NEXUSFLOW_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `nexusflow`, ou defina `NEXUSFLOW_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `nexusflow serve` quanto ao `nexusflow web`.

```bash
NEXUSFLOW_SERVER_PASSWORD=your-password nexusflow serve
```
...
Quando você executa `nexusflow`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor nexusflow para interagir com o nexusflow programaticamente.
:::

Essa arquitetura permite que o nexusflow suporte múltiplos clientes e permite que você interaja com o nexusflow programaticamente.

Você pode executar `nexusflow serve` para iniciar um servidor autônomo. Se você tiver o TUI do nexusflow em execução, `nexusflow serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do nexusflow [IDE](/docs/ide).
...
O servidor nexusflow expõe as seguintes APIs.
