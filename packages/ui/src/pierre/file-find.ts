...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("nexusflow-find")
  api.delete("nexusflow-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("nexusflow-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("nexusflow-find", new Highlight(...rest))
    return true
  }
