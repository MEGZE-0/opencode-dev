    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".nexusflow/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const nexusflowModel = {
      ...openaiModel,
      providerID: "nexusflow",
      api: {
        id: "nexusflow-test",
        url: "https://api.nexusflow.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, nexusflowModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.nexusflow?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.nexusflow?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          nexusflow: { itemId: "msg_nexusflow" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              nexusflow: { itemId: "msg_nexusflow_part" },
    ...
    expect(result[0].providerOptions?.nexusflow?.itemId).toBe("msg_nexusflow")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.nexusflow?.itemId).toBe("msg_nexusflow_part")
