...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.NEXUSFLOW_TEST ?? "")'), {
      env: {
        NEXUSFLOW_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "NEXUSFLOW_TEST_SHELL"], {
      shell: true,
      env: {
        NEXUSFLOW_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("NEXUSFLOW_TEST_SHELL=ok")
  })
