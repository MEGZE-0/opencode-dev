...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "nexusflow.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "nexusflow.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexusflow.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexusflow.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["nexusflow.json", "nexusflow.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "nexusflow.json"),
        path.join(project, "nexusflow.jsonc"),
      ])
    })
  })
