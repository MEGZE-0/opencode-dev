...
import { Project as ProjectV2 } from "@nexusflow/core/project"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const nexusflowFile = path.join(tmp, ".git", "nexusflow")
      expect(yield* Effect.promise(() => Bun.file(nexusflowFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "nexusflow")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "nexusflow")
      const wrongCache = path.join(parentDir, ".git", "nexusflow")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "nexusflow")
      const cacheB = path.join(bareB, "nexusflow")
      const wrongCache = path.join(parentDir, ".git", "nexusflow")
