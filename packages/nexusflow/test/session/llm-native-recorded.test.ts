...
import { ModelsDev } from "@nexusflow/core/models-dev"
import { LocationServiceMap } from "@nexusflow/core/location-layer"
import { HttpRecorder, Redactor } from "@nexusflow/http-recorder"
...
import { LLMEvent, LLMResponse } from "@nexusflow/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@nexusflow/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.nexusflow.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NEXUSFLOW_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["nexusflow", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEXUSFLOW_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NEXUSFLOW_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["nexusflow", "native", "oauth", "tool-loop"],
  ...
  {
    id: "nexusflow-proxy",
    name: "NexusFlow proxy",
    providerID: ProviderID.nexusflow,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["nexusflow", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NEXUSFLOW_RECORD_CONSOLE_TOKEN && process.env.NEXUSFLOW_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.nexusflow,
        name: "NexusFlow Zen",
        env: ["NEXUSFLOW_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NEXUSFLOW_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NEXUSFLOW_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NEXUSFLOW_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["nexusflow", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEXUSFLOW_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NEXUSFLOW_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NEXUSFLOW_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NEXUSFLOW_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the nexusflow LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "nexusflow.json"),
      JSON.stringify({ $schema: "https://nexusflow.ai/config.json", ...scenario.config(model) }),
    ),
  )
