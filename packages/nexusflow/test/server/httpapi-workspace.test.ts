...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-nexusflow-directory", directory)
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-nexusflow-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://nexusflow.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-nexusflow-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-nexusflow-workspace")
