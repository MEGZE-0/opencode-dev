// Subprocess test harness for the nexusflow CLI. Spawns the real binary against
...
//
// Configuration flows through nexusflow's built-in test affordances:
//   - NEXUSFLOW_CONFIG_CONTENT      : provider config inline, no files to find
//   - NEXUSFLOW_TEST_HOME           : pins os.homedir() → tmpdir
//   - NEXUSFLOW_DISABLE_PROJECT_CONFIG : skip walking up for nexusflow.json
//   - NEXUSFLOW_PURE                : skip external plugin discovery + install
//   - NEXUSFLOW_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `nexusflow.run` is fully wired. The shape supports adding more
// builders (`nexusflow.serve(opts)`, `nexusflow.acp(opts)`, `nexusflow.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { AppFileSystem } from "@nexusflow/core/filesystem"
import { AppProcess } from "@nexusflow/core/process"
...
const nexusflowRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(nexusflowRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    NEXUSFLOW_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    NEXUSFLOW_CONFIG_CONTENT: configJson,
    NEXUSFLOW_DISABLE_PROJECT_CONFIG: "1",
    NEXUSFLOW_PURE: "1",
    NEXUSFLOW_DISABLE_AUTOUPDATE: "1",
    NEXUSFLOW_DISABLE_AUTOCOMPACT: "1",
    NEXUSFLOW_DISABLE_MODELS_FETCH: "1",
...
// Typed equivalent of constructing argv for `nexusflow run`. New flags should
...
// `nexusflow serve` is a long-lived process — it never exits on its own.
...
// `nexusflow acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `nexusflow serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `nexusflow acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly nexusflow: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("nexusflow.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("nexusflow.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "nexusflow server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `nexusflow serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("nexusflow.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const nexusflow: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, nexusflow })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `nexusflow.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "nexusflow") {
...
// every call site. Use this for any test that needs the nexusflow CLI fixture.
//
...
// resources (e.g. `nexusflow.serve`) without an extra `Effect.scoped` wrapper —
