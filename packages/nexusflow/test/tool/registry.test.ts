...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".nexusflow")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .nexusflow/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const nexusflow = path.join(test.directory, ".nexusflow")
      const tool = path.join(nexusflow, "tool")
  ...
  it.instance("ignores non-tool exports in .nexusflow/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".nexusflow", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-nexusflow` was registering a tool
  ...
  it.instance("loads tools from .nexusflow/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const nexusflow = path.join(test.directory, ".nexusflow")
      const tools = path.join(nexusflow, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".nexusflow", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const nexusflow = path.join(test.directory, ".nexusflow")
        const customTools = path.join(nexusflow, "tools")
        const plugin = path.join(nexusflow, "node_modules", "@nexusflow-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(nexusflow, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@nexusflow/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".nexusflow", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(nexusflow, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@nexusflow/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(nexusflow, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(nexusflow, "node_modules", "cowsay")
