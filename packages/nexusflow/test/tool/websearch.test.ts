...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.NEXUSFLOW_WEBSEARCH_PROVIDER

    try {
      process.env.NEXUSFLOW_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.NEXUSFLOW_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.NEXUSFLOW_WEBSEARCH_PROVIDER
      else process.env.NEXUSFLOW_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for nexusflow or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderID.nexusflow, { exa: false, parallel: false })).toBe(true)
