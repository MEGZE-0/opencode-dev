...
it.instance("plan agent denies edits except .nexusflow/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".nexusflow/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".nexusflow", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.NEXUSFLOW_TEST_HOME
      process.env.NEXUSFLOW_TEST_HOME = test.directory
