...
describe("nexusflow acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ nexusflow }))
        ...
        expect(initialized.agentInfo?.name).toBe("NexusFlow")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ nexusflow })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("nexusflow-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "nexusflow-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.NEXUSFLOW_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexusflow }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("nexusflow-login")
