...
describe("nexusflow read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // NEXUSFLOW_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["mcp", "list"])
        nexusflow.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["providers", "list"])
        nexusflow.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["models"])
        nexusflow.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["agent", "list"])
        nexusflow.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh NEXUSFLOW_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["session", "list"])
        nexusflow.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["stats"])
        nexusflow.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under NEXUSFLOW_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ nexusflow }) =>
      Effect.gen(function* () {
        const r = yield* nexusflow.spawn(["db", "path"])
        nexusflow.expectExit(r, 0, "db path")
