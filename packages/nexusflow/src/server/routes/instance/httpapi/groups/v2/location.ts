...
import { PluginBoot } from "@nexusflow/core/plugin/boot"
...
>()("@nexusflow/ExperimentalHttpApiV2Location") {}

function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  const query = new URL(request.url, "http://localhost").searchParams
  return {
    directory: AbsolutePath.make(
      query.get("location[directory]") || request.headers["x-nexusflow-directory"] || process.cwd(),
    ),
    workspaceID: query.get("location[workspace]") || request.headers["x-nexusflow-workspace"],
  }
}
