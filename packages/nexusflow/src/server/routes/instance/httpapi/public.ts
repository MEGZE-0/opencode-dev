...
import { NexusFlowHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for nexusflow serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NexusFlowHttpApi)
...
export const PublicApi = NexusFlowHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "nexusflow",
    version: "1.0.0",
    description: "nexusflow api",
