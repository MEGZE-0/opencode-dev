...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall nexusflow and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall NexusFlow")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g nexusflow-ai",
      pnpm: "pnpm uninstall -g nexusflow-ai",
      bun: "bun remove -g nexusflow-ai",
      yarn: "yarn global remove nexusflow-ai",
      brew: "brew uninstall nexusflow",
      choco: "choco uninstall nexusflow",
      scoop: "scoop uninstall nexusflow",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "nexusflow", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using NexusFlow!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# nexusflow") || content.includes(".nexusflow/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".nexusflow/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".nexusflow/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".nexusflow"))
    ) {
      continue
    }
