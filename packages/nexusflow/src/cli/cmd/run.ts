// CLI entry point for `nexusflow run`.
//
// Handles three modes:
...
//      nexusflow server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@nexusflow/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run nexusflow with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running nexusflow server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NEXUSFLOW_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to NEXUSFLOW_SERVER_USERNAME or 'nexusflow')",
      })
      .option("dir", {
        type: "string",
