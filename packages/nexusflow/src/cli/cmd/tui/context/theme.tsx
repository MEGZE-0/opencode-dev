...
import nexusflow from "./theme/nexusflow.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@nexusflow/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "nexusflow")
        draft.active = typeof active === "string" ? active : "nexusflow"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.nexusflow, store.mode)
    })
