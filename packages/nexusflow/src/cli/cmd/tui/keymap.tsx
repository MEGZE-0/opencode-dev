...
export const NEXUSFLOW_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const NEXUSFLOW_MODE_KEY = "nexusflow.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(NEXUSFLOW_MODE_KEY, NEXUSFLOW_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(NEXUSFLOW_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(NEXUSFLOW_MODE_KEY, stack.at(-1)?.mode ?? NEXUSFLOW_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? NEXUSFLOW_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(NEXUSFLOW_MODE_KEY, undefined)
