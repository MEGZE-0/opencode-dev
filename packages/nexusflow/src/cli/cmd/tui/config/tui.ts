...
import type { DeepMutable } from "@nexusflow/core/schema"
import type { TuiAttentionSoundName } from "@nexusflow/plugin/tui"
...
export class Service extends Context.Service<Service, Interface>()("@nexusflow/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old nexusflow.json shape) still get their settings applied.
  ...
  // folders between cwd and home, and NEXUSFLOW_CONFIG_DIR/NEXUSFLOW_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NEXUSFLOW_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NEXUSFLOW_TUI_CONFIG override, if set.
  if (Flag.NEXUSFLOW_TUI_CONFIG) {
    const configFile = Flag.NEXUSFLOW_TUI_CONFIG
  ...
  // 4. Project config directories discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".nexusflow") || dir.endsWith(".nexusflow") || dir === Flag.NEXUSFLOW_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".nexusflow") && !dir.endsWith(".nexusflow") && dir !== Flag.NEXUSFLOW_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "nexusflow.default",
