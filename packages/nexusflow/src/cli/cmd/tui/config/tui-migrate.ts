...
const TUI_SCHEMA_URL = "https://nexusflow.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from nexusflow.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const nexusflow = await nexusflowFiles(input)
  for (const file of nexusflow) {
...
async function nexusflowFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "nexusflow"),
    ...ConfigPaths.fileInDirectory(Global.Path.config, "nexusflow"),
    ...(await Filesystem.findUp(
      ["nexusflow.json", "nexusflow.jsonc", "nexusflow.json", "nexusflow.jsonc"],
      input.cwd,
      undefined,
      { rootFirst: true },
    )),
  ...
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "nexusflow"))
    files.push(...ConfigPaths.fileInDirectory(dir, "nexusflow"))
  }
  if (Flag.NEXUSFLOW_CONFIG) files.push(Flag.NEXUSFLOW_CONFIG)
