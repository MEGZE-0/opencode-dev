...
//
// Enable with NEXUSFLOW_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/nexusflow/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.NEXUSFLOW_DIRECT_TRACE) {
    state = false
    return undefined
  }
