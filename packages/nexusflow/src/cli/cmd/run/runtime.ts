...
import { createOpencodeClient } from "@nexusflow/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "nexusflow.mode": input.resolveSession ? "local" : "attach",
      "nexusflow.initial_input": !!input.initialInput,
      "nexusflow.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "nexusflow.directory": ctx.directory,
        "nexusflow.resume": ctx.resume === true,
        "nexusflow.agent.name": state.agent,
        "nexusflow.model.provider": state.model?.providerID,
        "nexusflow.model.id": state.model?.modelID,
        "nexusflow.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "nexusflow.model.provider": model.providerID,
            "nexusflow.model.id": model.modelID,
            "nexusflow.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "nexusflow.model.id": state.model?.modelID,
                "nexusflow.model.variant": state.activeVariant,
                "nexusflow.prompt.chars": prompt.text.length,
                "nexusflow.prompt.parts": prompt.parts.length,
                "nexusflow.prompt.include_files": includeFiles,
                "nexusflow.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "nexusflow.directory": input.directory,
      "nexusflow.initial_input": !!input.initialInput,
      "nexusflow.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://nexusflow.internal",
        fetch: input.fetch,
