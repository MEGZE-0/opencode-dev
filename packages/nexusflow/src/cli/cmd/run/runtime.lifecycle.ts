...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "nexusflow.agent.name": input.agent,
      "nexusflow.directory": input.directory,
      "nexusflow.first": input.first,
      "nexusflow.model.provider": input.model?.providerID,
      "nexusflow.model.id": input.model?.modelID,
      "nexusflow.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "nexusflow.show_exit": next.showExit,
