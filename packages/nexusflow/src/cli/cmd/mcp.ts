...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: nexusflow mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in nexusflow.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  const candidates = [
    path.join(baseDir, "nexusflow.jsonc"),
    path.join(baseDir, "nexusflow.json"),
    path.join(baseDir, "nexusflow.jsonc"),
    path.join(baseDir, "nexusflow.json"),
  ]
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".nexusflow", "nexusflow.jsonc"),
      path.join(baseDir, ".nexusflow", "nexusflow.json"),
      path.join(baseDir, ".nexusflow", "nexusflow.jsonc"),
      path.join(baseDir, ".nexusflow", "nexusflow.json"),
    )
  }
  ...
  // Default to nexusflow.jsonc if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., nexusflow x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "nexusflow-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
