...
declare const NEXUSFLOW_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "nexusflow.db")
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  return path.join(Global.Path.data, `nexusflow-${safe}.db`)
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  if (Flag.NEXUSFLOW_DB) {
    if (Flag.NEXUSFLOW_DB === ":memory:" || path.isAbsolute(Flag.NEXUSFLOW_DB)) return Flag.NEXUSFLOW_DB
    return path.join(Global.Path.data, Flag.NEXUSFLOW_DB)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof NEXUSFLOW_MIGRATIONS !== "undefined"
        ? NEXUSFLOW_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof NEXUSFLOW_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
