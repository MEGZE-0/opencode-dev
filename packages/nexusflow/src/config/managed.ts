...
const MANAGED_PLIST_DOMAIN = "ai.nexusflow.managed"

// Keys injected by macOS/MDM into the managed plist that are not NexusFlow config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/nexusflow"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "nexusflow")
    default:
      return "/etc/nexusflow"
  }
}

export function managedConfigDir() {
  return process.env.NEXUSFLOW_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
