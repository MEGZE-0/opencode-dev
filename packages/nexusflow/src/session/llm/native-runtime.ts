...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@nexusflow/llm"
import type { LLMClientShape } from "@nexusflow/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("nexusflow"))
    return { type: "unsupported", reason: "provider is not openai, nexusflow, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @nexusflow/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains nexusflow-owned. The native runtime only adapts
      // the @nexusflow/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
