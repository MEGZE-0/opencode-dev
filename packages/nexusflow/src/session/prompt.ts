...
import * as EffectLogger from "@nexusflow/core/effect/logger"
...
import { SessionEvent } from "@nexusflow/core/session-event"
import { ModelV2 } from "@nexusflow/core/model"
import { ProviderV2 } from "@nexusflow/core/provider"
import { AgentAttachment, FileAttachment, ReferenceAttachment, Source } from "@nexusflow/core/session-prompt"
...
import { LLMEvent } from "@nexusflow/llm"
...
export class Service extends Context.Service<Service, Interface>()("@nexusflow/SessionPrompt") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      const cfg = yield* config.get()
      const textPart = input.parts.length === 1 && input.parts[0]?.type === "text" ? input.parts[0] : undefined
      const routed = AgentScheduler.plan({
        text: textPart?.text ?? "",
        agent: "general",
        background: true,
        model: { providerID: info.model.providerID, modelID: info.model.modelID },
      })
      const routedParts: PromptInput["parts"] =
        routed.length > 1
          ? [
              {
                type: "text" as const,
                text: [
                  input.parts[0]?.type === "text" ? input.parts[0].text : "",
                  "",
                  "<system-reminder>",
                  "NexusFlow routed the independent work items below to subagents. Use their results to produce one consolidated answer.",
                  "</system-reminder>",
                ].join("\n"),
              },
              ...routed.map((task) => ({
      ...
      const resolvedParts = yield* Effect.forEach(routedParts, resolvePart, { concurrency: "unbounded" }).pipe(
