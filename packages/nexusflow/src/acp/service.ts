...
import type { Message, OpencodeClient, SessionMessageResponse } from "@nexusflow/sdk/v2"
...
export const AuthMethodID = "nexusflow-login"
...
export class Service extends Context.Service<Service, Interface>()("@nexusflow/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `nexusflow auth login` in the terminal",
      name: "Login with nexusflow",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, nexusflow provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const nexusflowProvider = providers[ProviderID.make("nexusflow")]
  const nexusflowModel = nexusflowProvider ? Provider.sort(Object.values(nexusflowProvider.models))[0] : undefined
  if (nexusflowProvider && nexusflowModel) return { providerID: nexusflowProvider.id, modelID: nexusflowModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "NexusFlow service failure", service })
}
