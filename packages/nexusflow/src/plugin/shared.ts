...
export const DEPRECATED_PLUGIN_PACKAGES = ["nexusflow-openai-codex-auth", "nexusflow-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, nexusflowVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(nexusflowVersion) || semver.major(nexusflowVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.nexusflow
  if (typeof range !== "string") return
  if (!semver.satisfies(nexusflowVersion, range)) {
    throw new Error(`Plugin requires nexusflow ${range} but running ${nexusflowVersion}`)
  }
}
