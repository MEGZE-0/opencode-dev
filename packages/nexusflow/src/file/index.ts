...
import { NonNegativeInt, type DeepMutable } from "@nexusflow/core/schema"
...
export interface Interface {
  ...
  readonly create: (input: { path: string; type: "file" | "directory" }) => Effect.Effect<void, unknown>
  readonly rename: (input: { oldPath: string; newPath: string }) => Effect.Effect<void, unknown>
  readonly remove: (input: { path: string }) => Effect.Effect<void, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@nexusflow/File") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("File.create")(function* (input: { path: string; type: "file" | "directory" }) {
      const ctx = yield* InstanceState.context
      const targetPath = path.join(ctx.directory, input.path)
      if (!containsPath(targetPath, ctx)) {
        return yield* Effect.fail(new Error("Path is outside project directory"))
      }
      if (input.type === "directory") {
        yield* appFs.makeDirectory(targetPath, { recursive: true })
      } else {
        yield* appFs.writeFileString(targetPath, "")
      }
      yield* scan()
    })

    const rename = Effect.fn("File.rename")(function* (input: { oldPath: string; newPath: string }) {
      const ctx = yield* InstanceState.context
      const oldTarget = path.join(ctx.directory, input.oldPath)
      const newTarget = path.join(ctx.directory, input.newPath)
      if (!containsPath(oldTarget, ctx) || !containsPath(newTarget, ctx)) {
        return yield* Effect.fail(new Error("Path is outside project directory"))
      }
      yield* appFs.rename(oldTarget, newTarget)
      yield* scan()
    })

    const remove = Effect.fn("File.remove")(function* (input: { path: string }) {
      const ctx = yield* InstanceState.context
      const targetPath = path.join(ctx.directory, input.path)
      if (!containsPath(targetPath, ctx)) {
        return yield* Effect.fail(new Error("Path is outside project directory"))
      }
      yield* appFs.remove(targetPath, { recursive: true })
      yield* scan()
    })

    log.info("init")
    return Service.of({ init, status, read, list, create, rename, remove, search })
  }),
)
