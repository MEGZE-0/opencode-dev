...
Replace runtime Bun shell template-tag usage in `packages/nexusflow/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/nexusflow/src` is removed except:
