...
Delete `packages/nexusflow/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/nexusflow/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/nexusflow/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/nexusflow`
- TUI plugin loader tests in `packages/nexusflow` if runtime plugin API wiring changed
