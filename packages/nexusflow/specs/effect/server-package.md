...
Practical reference for a future `packages/server` split after the nexusflow
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/nexusflow`.
...
Until enough shared service code lives outside `packages/nexusflow`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/nexusflow`

It should not import `packages/nexusflow` services while `packages/nexusflow`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on nexusflow-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/nexusflow` implementation details.
