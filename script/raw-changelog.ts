...
const repo = process.env.GH_REPO ?? "anomalyco/nexusflow"
const bot = ["actions-user", "github-actions[bot]", "nexusflow", "nexusflow-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/nexusflow packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/nexusflow/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/nexusflow/")) areas.add("core")
