...
import { createOpencode } from "@nexusflow/sdk"
...
`)
  ...
  const nexusflow = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await nexusflow.client.session.create()
    const result = await nexusflow.client.session
      .prompt({
